...
void *PatchMgr::operator new(size_t size)
{
	sLog.outDebug("New PatchMgr dynamic string in object.");
	return malloc(size);
}

void PatchMgr::operator delete(void* p)
{
	sLog.outDebug("Delete PatchMgr dynamic string in object.");
	free(p);
}
