...
typedef struct Realm
{
	...
	AuthCommServerSocket *ServerSocket;
}Realm;
...
class InformationCore : public Singleton<InformationCore>
{
	...
	set<AuthCommServerSocket*> m_serverSockets;
...
public:
	...
	{ 
		realmhigh = 0;
		usepings  = !Config.MainConfig.GetBoolDefault("AuthServer", "DisablePings", false);
	}
	...
	void		  SetRealmOffline(uint32 realm_id, AuthCommServerSocket *ss);
	void		  UpdateRealmPop(uint32 realm_id, float pop);

	ARCTIC_INLINE void   AddServerSocket(AuthCommServerSocket * sock) { serverSocketLock.Acquire(); m_serverSockets.insert( sock ); serverSocketLock.Release(); }
	ARCTIC_INLINE void   RemoveServerSocket(AuthCommServerSocket * sock) { serverSocketLock.Acquire(); m_serverSockets.erase( sock ); serverSocketLock.Release(); }
