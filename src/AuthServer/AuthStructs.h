...
typedef struct {
	...
	uint8   os[4];		       // 'Win'
	uint8   country[4];	       // 'ruRU'
	uint32  WorldRegion_bias;  // -419
	uint32  ip;			       // client ip
	uint8   I_len;		       // length of account name
	uint8   I[50];		       // account name
} sAuthAuthChallenge_C;

typedef sAuthAuthChallenge_C sAuthReconnectChallenge_C;

typedef struct {
	uint8   cmd;			// 0x00 CMD_AUTH_Auth_CHALLENGE
	uint8   error;		    // 0 - ok
	uint8   unk2;		    // 0x00
	uint8   B[32];
	uint8   g_len;		    // 0x01
...
typedef struct {
	uint8   cmd;			// 0x01 CMD_AUTH_Auth_PROOF
	uint8   error;
	uint8   M2[20];
	uint32  unk2;
	uint16  unk203;
