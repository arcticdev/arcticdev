...
class AkamaAI : public MoonScriptBossAI
{
	...
	void ToIllidanEvent()
	{
		...
		if ((mScenePart <= 15 && pGate == NULL) || mScenePart == -1)
		{
			...
			if (mUdaloAI != NULL)
			{
				mUdaloAI->SetTargetToChannel(NULLUNIT, 0);
			}
			if (mOlumAI != NULL)
			{
	...
	void FightWithIllidan()
	{
		...
		if (mScenePart <= 2)
		{
			...
			else if (mIllidanAI->GetUnit()->GetAIInterface()->m_moveFly)
			{
				SetCanEnterCombat(false);
				_unit->GetAIInterface()->SetAIState(STATE_IDLE);
				_unit->GetAIInterface()->SetNextTarget(NULLUNIT);
		...
		else if (GetHealthPercent() <= 65 && !IsCasting())
		{
			...
			if (pAkama != NULL && pAkama->GetScript() != NULL)
			{
				...
				if (pAkamaAI->mScenePart <= 2 && pAkamaAI->GetCanEnterCombat())
				{
					...
					pAkamaAI->GetUnit()->GetAIInterface()->SetNextTarget(NULLUNIT);
			...
			if (GetRangeToUnit(pTarget) <= 80.0f)
			{
				...
				if (mFlameBurstTimer <= 0)
				{
					...
					for (unordered_set<PlayerPointer>::iterator itr = _unit->GetInRangePlayerSetBegin(); itr != _unit->GetInRangePlayerSetEnd(); ++itr) 
					{
						...
						if (pAI != NULL)
						{
							float Distance = GetRangeToUnit(*itr);
							if (Distance == 0.0f)
							{
		...
		if (pIllidan != NULL)
		{
			...
			else if (mHasTrapped || !pIllidan->isAlive())
			{
				...
				for (size_t i = 0; i < mTriggerAIList.size(); ++i)
				{
					...
					pTriggerAI->SetTargetToChannel(NULLUNIT, 0);
	...
	void AIUpdate()
	{
		...
		if (pBlade1 != NULL && pBlade2 != NULL && mChargeSpellFunc->mLastCastTime + mChargeSpellFunc->mCooldown <= (uint32)time(NULL))
		{
			...
			{	// && or || ? - not sure about details too
				...
				{
					ClearHateList();
					_unit->GetAIInterface()->SetNextTarget(*itr);
					_unit->GetAIInterface()->AttackReaction(*itr, 10000);
