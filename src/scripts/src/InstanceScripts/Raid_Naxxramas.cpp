...
void SpellFunc_MaexxnaWebWrap( SpellDesc* pThis, MoonScriptCreatureAI* pCreatureAI, UnitPointer pTarget, TargetType pType )
{
	...
	if ( Maexxna != NULL )
	{
		...
		if ( pTarget == NULLUNIT || !pTarget->IsPlayer() || pTarget->HasAura( MAEXXNA_WEB_WRAP ) || Maexxna->GetUnit() == NULLUNIT || Maexxna->GetUnit()->GetMapMgr() == NULLMAPMGR )
			return;
		...
		if ( pTarget->GetCurrentSpell() != NULLSPELL )
...
void SpellFunc_AnubRekhanCorpseScarabsPlayer( SpellDesc* pThis, MoonScriptCreatureAI* pCreatureAI, UnitPointer pTarget, TargetType pType )
{
	...
	if ( AnubRekhan != NULL )
	{
		...
		{ 
			...
			{
				if ( PlayerPtr->getMyCorpse() == NULLCORPSE || PlayerPtr->getMyCorpse()->GetCorpseState() != CORPSE_STATE_BODY )
					continue;
...
{
	if ( mShadeAI != NULL && mShadeAI->GetUnit()->GetAIInterface()->GetNextTarget() != NULLUNIT )
	{
