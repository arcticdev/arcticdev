...
namespace SUNWELL_KALECGOS
{
	...
	public:
		...
		void AIUpdate()
		{
			...
			if( !failed && _unit->FindAura(KALECGOS_BANISHED_AURA ) && sathrovar->FindAura(KALECGOS_BANISHED_AURA) && darkelf->isAlive() )
			{
				...
				cbMap.push_back(new CallBackManager(0x00,new NoSharedPtrCallbackP0<DRAGON_KALECGOS>(this,&DRAGON_KALECGOS::MoveToWps),5000,1,false,range));
			}
		}
		...
		void const InitializeSpells()
		{
			...
			CallBackManager * cbm = new CallBackManager(KALECGOS_FROST_BREATH,new NoSharedPtrCallbackP1<DRAGON_KALECGOS,uint32>(this,&DRAGON_KALECGOS::CastScriptSpell,KALECGOS_FROST_BREATH),10000,0,true,range);
			buffer.insert(cbm);
			cbm = new CallBackManager(KALECGOS_TAIL_LASH,new NoSharedPtrCallbackP1<DRAGON_KALECGOS,uint32>(this,&DRAGON_KALECGOS::CastScriptSpell,KALECGOS_TAIL_LASH),10000,0,true,range);
			buffer.insert(cbm);
			cbm = new CallBackManager(KALECGOS_WILDMAGIC,new NoSharedPtrCallbackP1<DRAGON_KALECGOS,uint32>(this,&DRAGON_KALECGOS::CastScriptSpell,KALECGOS_WILDMAGIC),10000,0,true,range);
			buffer.insert(cbm);
			range[0] = 15000;
			range[1] = 25000;
			cbm = new CallBackManager(KALECGOS_SPECTRAL_BLAST_DUMMY,new NoSharedPtrCallbackP1<DRAGON_KALECGOS,uint32>(this,&DRAGON_KALECGOS::CastScriptSpell,KALECGOS_SPECTRAL_BLAST_DUMMY),15000,0,true,range);
			buffer.insert(cbm);
			range[0] = 9000;
			range[1] = 0x00;
			cbm = new CallBackManager(KALECGOS_ARCANE_BUFFET,new NoSharedPtrCallbackP1<DRAGON_KALECGOS,uint32>(this,&DRAGON_KALECGOS::CastScriptSpell,KALECGOS_ARCANE_BUFFET),9000,0,false,range);
			buffer.insert(cbm);
			...
			case KALECGOS_SPECTRAL_BLAST_DUMMY:
				{
					...
					if(target != NULL)
					{
						...
						CallBackManager * cb = new CallBackManager(spellid,new NoSharedPtrCallbackP1<DRAGON_KALECGOS,uint32>(this,&DRAGON_KALECGOS::CastScriptSpell,KALECGOS_SPECTRAL_BLAST_AOE),2000,1,false,range);
						cbMap.push_back(cb);
					}
				}break;
	...
	public:
		...
		{
			uint16 range[] = {10000,12000};
			CallBackManager * cbm = new CallBackManager(SATHROVAR_SBOLT_VOLLEY,new NoSharedPtrCallbackP1<SATHROVAR,uint32>(this,&SATHROVAR::CastScriptSpell,SATHROVAR_SBOLT_VOLLEY),10000,0,true,range);
			cbMap.push_back(cbm);
			cbm = new CallBackManager(SATHROVAR_CORRUPTING_STRIKE,new NoSharedPtrCallbackP1<SATHROVAR,uint32>(this,&SATHROVAR::CastScriptSpell,SATHROVAR_CORRUPTING_STRIKE),10000,0,true,range);
			cbMap.push_back(cbm);
			range[0] = 60000;
			range[1] = 0;
			cbm = new CallBackManager(SATHROVAR_BOUNDLESS_CURSE,new NoSharedPtrCallbackP1<SATHROVAR,uint32>(this,&SATHROVAR::CastScriptSpell,SATHROVAR_BOUNDLESS_CURSE),60000,0,false,range);
			cbMap.push_back(cbm);
		}
		void const FillSayMap()
		{
	...
	public:
		...
		void const InitializeSpells()
		{
			...
			CallBackManager * cbm = new CallBackManager(DARKELF_HEROIC_STRIKE,new NoSharedPtrCallbackP1<DARK_ELF,uint32>(this,&DARK_ELF::CastScriptSpell,DARKELF_HEROIC_STRIKE),5000,0,true,range);
			cbMap.push_back(cbm);
			range[1] = 10000; 
			cbm = new CallBackManager(DARKELF_REVITALIZE,new NoSharedPtrCallbackP1<DARK_ELF,uint32>(this,&DARK_ELF::CastScriptSpell,DARKELF_REVITALIZE),5000,0,true,range);
			cbMap.push_back(cbm);
		}
		void const FillSayMap()
		{
