...
void *MoonInstanceScript::operator new(size_t size)
{
	sLog.outDebug("New MoonInstanceScript dynamic string in object.");
	return malloc(size);
}

void MoonInstanceScript::operator delete(void* p)
{
	sLog.outDebug("Delete MoonInstanceScript dynamic string in object.");
	free(p);
}
...
void MoonInstanceScript::AddGameObjectStateByEntry( uint32 pEntry, GameObjectState pState, bool pUseQuery )
{
	...
	if ( !pUseQuery )
	{
		...
		{
			CurrentObject = ( *Iter ).second;
			if ( CurrentObject == NULLGOB || CurrentObject->GetEntry() != pEntry )
				continue;
