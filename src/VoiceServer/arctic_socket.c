...
#include "arctic_packet.h"
#include "arctic_opcodes.h"
#include "voice_channel.h"
#include "voice_channelmgr.h"

#define ARCTICSOCKET_RBUF_LEN 5000
...
arctic_socket * arcticsocket_allocate(network_socket *s)
{
	arctic_socket * ret = (arctic_socket*)vc_malloc(sizeof(arctic_socket));
	ret->read_buf = (char*)vc_malloc(ARCTICSOCKET_RBUF_LEN);
	ret->read_buf_len = 0;
	ret->read_buf_sz = ARCTICSOCKET_RBUF_LEN;
	ret->sock = s;
	ret->channelcount = 0;
	++g_serverCount;
	return ret;
...
void arcticsocket_free(arctic_socket* s)
{
	if( s->channelcount > 0 )
...
void arcticsocket_send(arctic_socket *s, arctic_packet *p)
{
...
int voicechat_arctic_listen_socket_read_handler(network_socket *s, int act)
{
	...
	ns->event_handler = voicechat_arctic_socket_read_handler;
	ns->miscdata = (void*)arcticsocket_allocate(ns);		// used for voicechat struct later
	...
	if( network_add_socket(ns) < 0 )
	{
		...
		arcticsocket_free((arctic_socket*)ns->miscdata);
		network_close(ns);
	}
...
// client handlers
void vc_handler_createchannel(arctic_socket *s, arctic_packet *p)
{
	uint8 type;
	uint32 request_id;
	arctic_packet reply;
	...
	type = arcticpacket_readu8(p);
	request_id = arcticpacket_readu32(p);
	...
	if( chn == NULL )
	{
		...
		arcticpacket_init(VOICECHAT_SMSG_CHANNEL_CREATED, 5, &reply);
		arcticpacket_writeu32(&reply, request_id);
		arcticpacket_writeu8(&reply, error);

		// send reply
		arcticsocket_send(s, &reply);

		// free buffer
		arcticpacket_free(&reply);
	}
	else
	{
		...
		arcticpacket_init(VOICECHAT_SMSG_CHANNEL_CREATED, 7, &reply);
		arcticpacket_writeu32(&reply, request_id);
		arcticpacket_writeu8(&reply, error);
		arcticpacket_writeu16(&reply, reply_id);
...
void vc_handler_deletechannel(arctic_socket *s, arctic_packet *p)
{
	uint16 channel_id;
	voice_channel *ch;

	channel_id = arcticpacket_readu16(p);
...
void vc_handler_addmember(arctic_socket *s, arctic_packet *p)
{
	uint16 channel_id;
	uint8 member_id;
	voice_channel *ch;
	...
	channel_id = arcticpacket_readu16(p);
	member_id = arcticpacket_readu8(p);
...
void vc_handler_deletemember(arctic_socket *s, arctic_packet *p)
{
	uint16 channel_id;
	uint8 member_id;
	voice_channel *ch;
...
void vc_handler_ping(arctic_socket *s, arctic_packet *p)
{
	arctic_packet bp;
	uint32 pi;

	printf("ping!\n");
	pi = arcticpacket_readu32(p);

	arcticpacket_init(VOICECHAT_SMSG_PONG, 4, &bp);
	arcticpacket_writeu32(&bp, pi);
	arcticsocket_send(s, &bp);
	arcticpacket_free(&bp);
}

// handler table
typedef void(*vc_handler)(arctic_socket*,arctic_packet*);
...
{
	arctic_socket * mysock = (arctic_socket*)s->miscdata;
	uint16 opcode;
	uint16 len;
	int readcount;
	arctic_packet * pck;
	...
	for(;;)
	{
		...
		pck = arcticpacket_create(opcode, len);
