...
arctic_socket * ascentsocket_allocate(network_socket *s)
{
...
void ascentsocket_free(arctic_socket* s)
{
	if( s->channelcount > 0 )
...
void ascentsocket_send(arctic_socket *s, arctic_packet *p)
{
...
int voicechat_arctic_listen_socket_read_handler(network_socket *s, int act)
{
	...
	if( network_add_socket(ns) < 0 )
	{
		...
		ascentsocket_free((arctic_socket*)ns->miscdata);
		network_close(ns);
	}
