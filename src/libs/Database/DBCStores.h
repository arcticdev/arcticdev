...
public:
	...
	void ReadEntry(FILE * f, T * dest, const char * format, uint32 cols, const char * filename)
	{
		...
		if(len != cols)
		{
			...
			printf("!!! Core will pause for 20 seconds\n");
			Sleep(20000);
			return;
		}
	...
	ARCTIC_INLINE uint32 GetMaxRow()
	{
		return m_max;
	}
...
extern ARCTIC_DECL DBCStorage<CurrencyTypesEntry> dbcCurrencyTypesStore;
...
extern ARCTIC_DECL DBCStorage<ScalingStatDistributionEntry> dbcScalingStatDistribution;
extern ARCTIC_DECL DBCStorage<ScalingStatValuesEntry> dbcScalingStatValues;
