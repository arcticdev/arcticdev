...
class ARCTIC_DECL ThreadController
{
public:
	HANDLE hThread;
	uint32 thread_id;
...
public:
	...
	ARCTIC_INLINE uint32 GetActiveThreadCount() { return (uint32)m_activeThreads.size()+1; }

	// gets free thread count
	ARCTIC_INLINE uint32 GetFreeThreadCount() { return (uint32)m_freeThreads.size(); }
};

extern ARCTIC_DECL CThreadPool ThreadPool;
