...
void *WeatherMgr::operator new(size_t size)
{
	sLog.outDebug("New WeatherMgr dynamic string in object.");
	return malloc(size);
}

void WeatherMgr::operator delete(void* p)
{
	sLog.outDebug("Delete WeatherMgr dynamic string in object.");
	free(p);
}
...
//Конструктор и деструктор для динамических строк в объектах
void *WeatherInfo::operator new(size_t size)
{
	sLog.outDebug("New WeatherInfo dynamic string in object.");
	return malloc(size);
}

void WeatherInfo::operator delete(void* p)
{
	sLog.outDebug("Delete WeatherInfo dynamic string in object.");
	free(p);
}
