...
void *EventableObject::operator new(size_t size)
{
	sLog.outDebug("New EventableObject dynamic string in object.");
	return malloc(size);
}

void EventableObject::operator delete(void* p)
{
	sLog.outDebug("Delete EventableObject dynamic string in object.");
	free(p);
}
