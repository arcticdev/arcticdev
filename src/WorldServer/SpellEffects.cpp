...
pSpellEffect SpellEffectsHandler[TOTAL_SPELL_EFFECTS] =
{
	...
	&Spell::SpellEffectNULL,                        //  39 SPELL_EFFECT_LANGUAGE 
	&Spell::SpellEffectDualWield,					//  40 SPELL_EFFECT_DUAL_WIELD 
	&Spell::SpellEffectNULL,						//  41 SPELL_EFFECT_SUMMON_WILD 
	&Spell::SpellEffectMegaJump,					//  42 SPELL_EFFECT_SUMMON_GUARDIAN 
	...
	&Spell::SpellEffectNULL,						//  51 SPELL_EFFECT_FORCE_CRITICAL_HIT
	&Spell::SpellEffectNULL,						//  52 SPELL_EFFECT_GUARANTEE_HIT 
	...
	&Spell::SpellEffectNULL,						//  89 SPELL_EFFECT_SUMMON_TOTEM_SLOT3 
	&Spell::SpellEffectNULL,						//  90 SPELL_EFFECT_SUMMON_TOTEM_SLOT4 
	...
	&Spell::SpellEffectNULL,						//  93 SPELL_EFFECT_SUMMON_PHANTASM - OLD
	...
	&Spell::SpellEffectNULL,						//  97 SPELL_EFFECT_SUMMON_CRITTER 
	...
	&Spell::SpellEffectNULL,						// 125 unknown - Reduce Threat by % //http://www.thottbot.com/?sp=32835
	&Spell::SpellEffectSpellSteal,					// 126 SPELL_EFFECT_SPELL_STEAL - Steal Beneficial Buff (Magic) //http://www.thottbot.com/?sp=30449
	&Spell::SpellEffectProspecting,					// 127 unknown - Search 5 ore of a base metal for precious gems.  This will destroy the ore in the process.
	&Spell::SpellEffectApplyAura128,				// 128 unknown - Adjust a stats by %: Mod Stat // ITS FLAT
	&Spell::SpellEffectNULL,						// 129 unknown - Mod Dmg % (Spells)
	&Spell::SpellEffectNULL,						// 130 unknown - http://www.thottbot.com/s34477
	&Spell::SpellEffectNULL,						// 131 unknown - test spell
	&Spell::SpellEffectNULL,						// 132 unknown - no spells
	&Spell::SpellEffectNULL,						// 133 SPELL_EFFECT_FORGET_SPECIALIZATION - http://www.thottbot.com/s36441 // I think this is a gm/npc spell
	&Spell::SpellEffectKillCredit,                  // 134 SPELL_EFFECT_KILL_CREDIT - misc value is creature entry
	&Spell::SpellEffectNULL,						// 135 unknown - no spells
	&Spell::SpellEffectRestoreHealthPct,			// 136 Restore Health % - http://www.wowhead.com/?spell=48982
	&Spell::SpellEffectRestoreManaPct,				// 137 Restore Mana % - http://www.thottbot.com/s41542
	&Spell::SpellEffectNULL,						// 138 unknown - related to superjump or even "*jump" spells http://www.thottbot.com/?e=Unknown%20138
	&Spell::SpellEffectNULL,						// 139 unknown - no spells
	&Spell::SpellEffectTeleportUnits,				// 140 SPELL_EFFECT_TELEPORT_UNITS - IronForge teleport / portal only it seems
	&Spell::SpellEffectNULL,						// 141 unknown - triggers spell, magic one,  (Mother spell) http://www.thottbot.com/s41065
	&Spell::SpellEffectTriggerSpellWithValue,		// 142 unknown - triggers some kind of "Put spell on target" thing... (dono for sure) http://www.thottbot.com/s40872 and http://www.thottbot.com/s33076
	&Spell::SpellEffectNULL,						// 143 Apply area aura // Master -> deamon effecting spell, http://www.thottbot.com/s25228 and http://www.thottbot.com/s35696
	&Spell::SpellEffectNULL,						// 144 unknown - Spectral Blast
	&Spell::SpellEffectNULL,						// 145 unknown - Black Hole Effect
	&Spell::SpellEffectNULL,						// 146 unknown - unused
	&Spell::SpellEffectNULL,						// 147 unknown - Torch Tossing Training Failure
	&Spell::SpellEffectNULL,						// 148 unknown - unused
	&Spell::SpellEffectNULL,						// 149 unknown 
	&Spell::SpellEffectNULL,						// 150 unknown - unused
	&Spell::SpellEffectTriggerSpell,				// 151 SPELL_EFFECT_TRIGGER_SPELL_2
	&Spell::SpellEffectNULL,						// 152 unknown 
	...
	&Spell::SpellEffectRenamePet,					// 159 allow rename pet once again
...
void Spell::SpellEffectSchoolDMG(uint32 i) // dmg school
{
	...
	if(m_spellInfo->EffectChainTarget[i])//chain
	{
		if( m_spellInfo->Id == 53595 )
		{
			...
			float WMAX = 1.0f;
			float MWS = 1.0f;
			ItemPointer it = p_caster->GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_MAINHAND);
			if( it && it->GetProto() )
			{
			...
			case SPELL_HASH_SHADOWFLAME:
				{
					...
					SpellPointer sp (new Spell( u_caster, SpellInfo, true, NULLAURA ));
		...
		case SPELL_HASH_STEADY_SHOT:
			{
				if( p_caster != NULL )
				{
					...
					if( it )
						dmg += float2int32(it->GetProto()->Damage[0].Min + rand()%float2int32(it->GetProto()->Damage[0].Max - it->GetProto()->Damage[0].Min)); //+unmodified weapon damage

					ItemPrototype* ip = ItemPrototypeStorage.LookupEntry( p_caster->GetUInt32Value( PLAYER_AMMO_ID ) );
					if( ip )
						dmg += float2int32(ip->Damage[0].Min + rand()%(float2int32(ip->Damage[0].Max - ip->Damage[0].Min)+1)); //+unmodified weapon damage
				}
			}break;
		case SPELL_HASH_EVISCERATE:
			{
				if( p_caster != NULL )
				{
					...
					{
						AuraPointer aur = p_caster->FindPositiveAuraByNameHash(SPELL_HASH_SLICE_AND_DICE);
						if( aur )
						{
	...
	case SPELL_HASH_VANISH:
		{
			...
			if( p_caster )
			{
				for( uint32 x = MAX_POSITIVE_AURAS ; x < MAX_AURAS ; x++ )
				{
					if( p_caster->m_auras[x] && !p_caster->m_auras[x]->IsPositive())
					{
	...
	case 34300:
		{
			...
			ILotP.procFlags = PROC_ON_RANGED_ATTACK | PROC_ON_MELEE_ATTACK | PROC_TARGET_SELF;
			ILotP.ExtraProc = EXTRA_ON_CRIT;
			ILotP.deleted = false;
			...
			if( p_caster->GetGroup() != NULL )
			{
				...
				if( grp->m_prayerOfMendingCount > 0 )
				{
					...
					if( possible_targets.size() > 0 )
					{
						...
						// cast us on them
						SpellPointer nsp(new Spell(p_caster, dbcSpell.LookupEntry(mspell->Id), true, NULLAURA));
	...
	case 49895: // Death Coil
		{
			if( u_caster == NULL || unitTarget == NULL ) 
				return;
			if( spellId == 52375 )
				damage = damage * 2 / 5;	// 40% Первый ранг...
		...
		case 48743://death pact
			{
				...
				uint32 curhp = p_caster->GetHealth();
				int32 realdmg = float2int32((float)(curhp * 0.4f));
				Heal( realdmg );
			}break;
		case 50464:
			{
		...
		case 20267: // judgement of light heal effect
			{
				if( u_caster )
				{
					AuraPointer aur = u_caster->FindAura(20185);
					if( aur )
					{
						UnitPointer orgcstr = u_caster->FindAura(20185)->GetUnitCaster();
						if( orgcstr )
...
void Spell::SpellEffectCreatePet(uint32 i)
{
    ...
    if( ci )
    {
        ...
        pPet->CreateAsSummon( GetProto()->EffectMiscValue[i], ci, NULLCREATURE, playerTarget, GetProto(), 1, 0 );
    }
}
...
void Spell::SpellEffectPersistentAA(uint32 i) // Persistent Area Aura
{
	...
	if(g_caster && g_caster->IsInWorld() && g_caster->m_summoner)
	{
		...
		if(!unitTarget)
		{
			dynObj->Create(g_caster->m_summoner, shared_from_this(), g_caster->GetPositionX(), g_caster->GetPositionY(), 
				g_caster->GetPositionZ(), dur, r);
		}
		else
		{
			dynObj->Create(g_caster->m_summoner, shared_from_this(), unitTarget->GetPositionX(), unitTarget->GetPositionY(),
	...
	case TARGET_FLAG_UNIT:
		{
			if(!unitTarget||!unitTarget->isAlive())
				break;
			dynObj->Create(m_caster, shared_from_this(), unitTarget->GetPositionX(), unitTarget->GetPositionY(), unitTarget->GetPositionZ(),
				dur, r);
		}break;
	...
	case TARGET_FLAG_SOURCE_LOCATION:
		{
			...
			dynObj->Create(m_caster, shared_from_this(), m_targets.m_srcX,
				m_targets.m_srcY, m_targets.m_srcZ, dur,r);
		}break;
	case TARGET_FLAG_DEST_LOCATION:
		{
			dynObj->SetInstanceID(m_caster->GetInstanceID());
			dynObj->Create(m_caster, shared_from_this(),
...
void Spell::SpellEffectDispel(uint32 i) // Dispel
{
	...
	if(unitTarget->m_auras[x])
	{
		...
		if(!aur->IsPassive() && !(aur->GetSpellProto()->Attributes & ATTRIBUTES_IGNORE_INVULNERABILITY))
		{
			...
			UnitPointer caster = aur->GetUnitCaster();
			if( caster )
			...
			if( !Rand(resistchance) )
			{
				...
				{
					unitTarget->HandleProc( PROC_ON_AURA_REMOVE, EXTRA_ON_DISPEL, u_caster, m_spellInfo, aur->GetSpellId() );
					data.clear();
					data << m_caster->GetNewGUID();
...
{
	if ( !u_caster || !u_caster->GetMapMgr())
		return;
	...
	if( i != 0 && m_spellInfo->Effect[i-1] == SPELL_EFFECT_APPLY_AURA && m_spellInfo->EffectApplyAuraName[i-1] == SPELL_AURA_MOUNTED )
	{
		...
		VehiclePointer veh = u_caster->GetMapMgr()->CreateVehicle( cr_entry );
		...
		for( uint8 d = 0; d < damage; d++ )
 		{
			...
			if ( g_caster ) 
			{
				...
				u_caster->m_SummonSlots[ d ] = TO_CREATURE(g_caster->CreateTemporaryGuardian(cr_entry,GetDuration(),m_fallowAngle, u_caster));
			}
			else
				u_caster->m_SummonSlots[ d ] = TO_CREATURE(u_caster->CreateTemporaryGuardian(cr_entry,GetDuration(),m_fallowAngle,level));

			//expire this slot
			sEventMgr.AddEvent(u_caster, &Unit::SummonExpireSlot, d, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT, GetDuration(), 1,0);
		}
	}
}
...
void Spell::SpellEffectSummonObject(uint32 i)
{
	...
	if( entry == GO_FISHING_BOBBER )
	{
		...
		sEventMgr.AddEvent( go, &GameObject::EndFishing, TO_PLAYER( m_caster ),true, EVENT_GAMEOBJECT_END_FISHING, 20000, 1, 0 );
...
void Spell::SpellEffectTameCreature(uint32 i)
{
	...
	if(!tame || !p_caster || !p_caster->isAlive() || !tame->isAlive() || p_caster->getClass() != HUNTER )
		...
		result = SPELL_FAILED_BAD_TARGETS;
	else if(tame->GetCreatureName()->Type != UNIT_TYPE_BEAST)
		result = SPELL_FAILED_BAD_TARGETS;
...
void Spell::SpellEffectInterruptCast(uint32 i) // Interrupt Cast
{
	...
	if(school)//prevent from casts in this school
	{
		...
		{		
			...
			int32 DurationModifier = TO_PLAYER( unitTarget )->MechanicDurationPctMod[MECHANIC_INTERRUPTED];
...
void Spell::SpellEffectPickpocket(uint32 i) // pickpocket
{
	...
	if(target->IsPickPocketed() || (target->GetCreatureName() && target->GetCreatureName()->Type != UNIT_TYPE_HUMANOID))
	{
...
void Spell::SpellEffectHealMechanical(uint32 i)
{
	...
	if(unitTarget->GetTypeId() != TYPEID_UNIT)
		return;
	if(TO_CREATURE(unitTarget)->GetCreatureName()->Type != UNIT_TYPE_MECHANICAL)
		return;
	...
	case 50725://vigilance
		{
			...
			AuraPointer aur = p_caster->FindAura( 50720 );
			if( aur )
			{
				UnitPointer caster = aur->GetUnitCaster();
	...
	case 47948://Pain and Suffering
		{
			...
			UnitPointer pstarget = GetUnitTarget();
			AuraPointer aur = pstarget->FindNegativeAuraByNameHash(SPELL_HASH_SHADOW_WORD__PAIN);
			if( aur )
	...
	case 47951:
		{
			...
			SpellPointer pSpell_a (new Spell(p_caster, sp, true, NULLAURA));
			pSpell_a->GenerateTargets( &targets );
			pSpell_a->prepare( &targets );
			
			sp = dbcSpell.LookupEntryForced( 47953 );
			SpellPointer pSpell_b (new Spell(p_caster, sp, true, NULLAURA));
	...
	case 53408:
		{
			...
			SpellEntry *en = dbcSpell.LookupEntry(spellid);
			SpellPointer sp(new Spell(p_caster, en, true, NULLAURA));
			SpellCastTargets tgt;
			tgt.m_unitTarget=unitTarget->GetGUID();
						...
						for(uint32 x = 0; x < p_caster->GetGroup()->GetSubGroupCount(); ++x)
						{
							...
							for(GroupMembersSet::iterator itr = p_caster->GetGroup()->GetSubGroup( x )->GetGroupMembersBegin(); itr != p_caster->GetGroup()->GetSubGroup( x )->GetGroupMembersEnd(); ++itr)
							{
								if((*itr)->m_loggedInPlayer && TargetCount <= 10)
								{
									...
									SpellPointer pSpell(new Spell(p_caster, Replinishment, true, NULLAURA));
									SpellCastTargets tgt;
...
void Spell::SpellEffectSanctuary(uint32 i) // Stop all attacks made to you
{
	...
	if( unitTarget->GetCurrentSpell() != NULL && unitTarget->GetCurrentSpell() != shared_from_this() && unitTarget->GetCurrentSpell()->getState() == SPELL_STATE_PREPARING )
...
void Spell::SpellEffectStuck(uint32 i)
{
	...
	if (playerTarget->GetSession())
		playerTarget->GetSession()->SetPortVec(playerTarget->GetBindMapId(), 0, LocationVector(playerTarget->GetBindPositionX(), playerTarget->GetBindPositionY(), playerTarget->GetBindPositionZ()));
}
...
void Spell::SummonTotem(uint32 i) // Summon Totem
{
	...
	pTotem->SetTotemOwner(TO_PLAYER(u_caster));
...
{
	if( unitTarget == NULL || !m_caster->IsPlayer() )
		return;

	CreaturePointer cr = TO_CREATURE( unitTarget );
...
void Spell::SpellEffectSummonObjectSlot(uint32 i)
{
	...
	GameObjectInfo * goi = GameObjectNameStorage.LookupEntry(m_spellInfo->EffectMiscValue[i]);
	if( goi == NULL )
	{
		if( p_caster != NULL )
		{
			sChatHandler.BlueSystemMessage(p_caster->GetSession(),
			"non-existant gameobject %u tried to be created by SpellEffectSummonObjectSlot. Report to devs!", m_spellInfo->EffectMiscValue[i]);
		}
		return;
	}
...
{ 
	if( !unitTarget || !unitTarget->IsPet() ||  
		 !TO_PET(unitTarget)->GetPetOwner() || TO_PET(unitTarget)->GetPetOwner()->getClass() != HUNTER ) 
         return; 
