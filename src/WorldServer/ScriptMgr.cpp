...
void *ScriptMgr::operator new(size_t size)
{
	sLog.outDebug("New ScriptMgr dynamic string in object.");
	return malloc(size);
}

void ScriptMgr::operator delete(void* p)
{
	sLog.outDebug("Delete ScriptMgr dynamic string in object.");
	free(p);
}
