...
void *TaxiPath::operator new(size_t size)
{
	sLog.outDebug("New TaxiPath dynamic string in object.");
	return malloc(size);
}

void TaxiPath::operator delete(void* p)
{
	sLog.outDebug("Delete TaxiPath dynamic string in object.");
	free(p);
}

//Конструктор и деструктор для динамических строк в объектах
void *TaxiMgr::operator new(size_t size)
{
	sLog.outDebug("New TaxiMgr dynamic string in object.");
	return malloc(size);
}

void TaxiMgr::operator delete(void* p)
{
	sLog.outDebug("Delete TaxiMgr dynamic string in object.");
	free(p);
}
