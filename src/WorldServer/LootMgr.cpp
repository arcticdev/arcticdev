...
void *LootRoll::operator new(size_t size)
{
	sLog.outDebug("New LootRoll dynamic string in object.");
	return malloc(size);
}

void LootRoll::operator delete(void* p)
{
	sLog.outDebug("Delete LootRoll dynamic string in object.");
	free(p);
}

//Конструктор и деструктор для динамических строк в объектах
void *LootMgr::operator new(size_t size)
{
	sLog.outDebug("New LootMgr dynamic string in object.");
	return malloc(size);
}

void LootMgr::operator delete(void* p)
{
	sLog.outDebug("Delete LootMgr dynamic string in object.");
	free(p);
}
