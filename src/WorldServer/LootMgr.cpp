...
{	
	...
	if(result)
	{
		...
		do
		{
			...
			if(rp == NULL)
			{
				Log.Error("LoadLoot", "RandomProp group %u references non-existant randomprop %u.", id, eid);
				continue;
			}
	...
	if(result)
	{
		...
		do 
		{
			...
			if(rs == NULL)
			{
				Log.Error("LoadLoot", "RandomSuffix group %u references non-existant randomsuffix %u.", id, eid);
				continue;
			}
