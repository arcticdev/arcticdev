...
void *MapMgr::operator new(size_t size)
{
	sLog.outDebug("New MapMgr dynamic string in object.");
	return malloc(size);
}

void MapMgr::operator delete(void* p)
{
	sLog.outDebug("Delete MapMgr dynamic string in object.");
	free(p);
}
...
bool MapMgr::Do()
{
	...
	for(CreatureSpawnList::iterator itr = _map->staticSpawns.CreatureSpawns.begin(); itr != _map->staticSpawns.CreatureSpawns.end(); ++itr)
	{
		...
		obj->Load(*itr, this->iInstanceMode, pMapInfo);
		_mapWideStaticObjects.insert(obj);
	}
...
VehiclePointer MapMgr::CreateVehicle(uint32 entry)
{
	...
	v->Init();
	ASSERT( v->GetTypeFromGUID() == HIGHGUID_TYPE_VEHICLE );
	m_VehicleStorage.insert( make_pair< const uint32, VehiclePointer >(v->GetUIdFromGUID(), v));
	return v;
}
...
CreaturePointer MapMgr::CreateCreature(uint32 entry)
{
	...
	CreaturePointer cr(new Creature(newguid));
	cr->Init();
	m_CreatureStorage.insert( make_pair< const uint32, CreaturePointer >(cr->GetUIdFromGUID(), cr));
	return cr;
}
