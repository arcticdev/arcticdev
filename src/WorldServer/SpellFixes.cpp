	...
	if(sp != NULL)
	{
		...
		{					
			...
			**********************************************************/
			...
			case 58357: //Glyph of Heroic Strike
				{
						sp->Effect[0] = 6;
						sp->EffectApplyAuraName[0] = 42;
				}break;

			case 54754: //Glyph of Rejuvenation
				{
					sp->Effect[0] = SPELL_EFFECT_APPLY_AURA;
            ...
            // http://ru.wowhead.com/?spell=19506 Аура меткого выстрела
			case 19506: 
				{
			...
			case 44472:
				{
						...
						sp->EffectImplicitTargetA[1] = 1;
						sp->EffectSpellClassMask[1][0] = 551555703;
						sp->EffectSpellClassMask[1][1] = 102472;
			...
			case 18073:
				{
						sp->Effect[0] = SPELL_EFFECT_APPLY_AURA;
						sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
						sp->EffectTriggerSpell[0] = 18093; // CHANGE IN 3.1 TO 63243
						sp->Effect[1] = 0;
				}break;			
			case 63245:
				{
						sp->Effect[0] = SPELL_EFFECT_APPLY_AURA;
						sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
						sp->EffectTriggerSpell[0] = 18093; // CHANGE IN 3.1 TO 63244
						sp->Effect[1] = 0;
				}break;
			...
			case 16857:
				{
					...
					sp->EffectImplicitTargetA[2] = 6;
					sp->School = 8;
					sp->EffectChainTarget[2] = 0;
				}break;
...
//		    type |= SPELL_TYPE_WARLOCK_CURSES;
		...
		if( sp->NameHash == SPELL_HASH_DEEP_WOUNDS )
			sp->SpellGroupType[1] = 16;
		...
		if( sp->NameHash == SPELL_HASH_DEATH_COIL )
			sp->EffectMultipleValue[0] = 3;
		...
		if( sp->NameHash == SPELL_HASH_WILD_GROWTH )
			sp->RequiredShapeShift = 2;

		if( sp->NameHash == SPELL_HASH_FAERIE_FIRE )
			sp->c_is_flags |= SPELL_FLAG_IS_FORCEDDEBUFF;
	...
	//////////////////////////////////////////////////////////////////
	QueryResult * resultx = WorldDatabase.Query("SELECT * FROM spell_forced_targets");
	if( resultx != NULL )
	{
		do 
		{
	...
	resultx = WorldDatabase.Query("SELECT * FROM spell_coef_override");
	if( resultx != NULL )
	{
		uint32 spellid;
		do 
		{
			...
			if( sp != NULL )
			{
				sp->Base_coef = f[1].GetFloat();
				sp->Misc_coef = f[2].GetFloat();
				sp->AP_coef = f[3].GetFloat();
				sp->RAP_coef = f[4].GetFloat();
			}
			else
			{
