...
void *LogonCommHandler::operator new(size_t size)
{
	sLog.outDebug("New LogonCommHandler dynamic string in object.");
	return malloc(size);
}

void LogonCommHandler::operator delete(void* p)
{
	sLog.outDebug("Delete LogonCommHandler dynamic string in object.");
	free(p);
}
...
void LogonCommHandler::Startup()
{
	...
	Log.Notice(LOGONCOMMCLIENTSAI, LOADINFORCLIENTSAI);
...
{
	Log.Notice(LOGONCOMMCLIENTSAI, ATTAMTINGCONNCTSAI);
...
void LogonCommHandler::Connect(LogonServer * server)
{
	...
	Log.Notice(LOGONCOMMCLIENTSAI, ZATTEMPSOURLOGONAI, ReConCounter, server->Address.c_str(), server->Port );
	...
	if(conn == 0)
	{
		Log.Notice(LOGONCOMMCLIENTSAI, CONNECTIONFAILEDAI);
		return;
	}
	Log.Notice(LOGONCOMMCLIENTSAI, AUTHECATIONSKHYEAI);
	...
	while(!conn->authenticated)
	{
		...
		{
			Log.Notice(LOGONCOMMCLIENTSAI, AUTHECATIONSTIMEAI);
			conn->Disconnect();
			logons[server]=NULL;
			return;
		}
	...
	{
		Log.Notice(LOGONCOMMCLIENTSAI, AUTHECATIONHJKIEAI);
		logons[server] = NULL;
		conn->Disconnect();
		return;
	}
	else
		Log.Notice(LOGONCOMMCLIENTSAI, AUTHECATIONHDSIEAI);

	// Send the initial ping
	conn->SendPing();

	Log.Notice(LOGONCOMMCLIENTSAI, REGISCATIONHDSIEAI);
	conn->_id = server->ID;
	...
	while(server->Registered == false)
	{
		...
		{
			Log.Notice(LOGONCOMMCLIENTSAI, REGISCATIONHDOUTAI);
			logons[server] = NULL;
			conn->Disconnect();
			break;
		}
	...
	Log.Notice(LOGONCOMMCLIENTSAI, LOGONSERVERSALATAI, conn->latency);
	...
	for(; itr != logons.end(); ++itr)
	{
		...
		if(cs != 0)
		{
			...
			if(cs->last_pong < t && ((t - cs->last_pong) > 60))
			{
				...
				printf(LOGONSDROPPERSATAI, (unsigned int)itr->first->ID);
				cs->_id = 0;
				cs->Disconnect();
				itr->second = 0;
				continue;
...
void LogonCommHandler::ConnectionDropped(uint32 ID)
{
	...
	for(; itr != logons.end(); ++itr)
	{
		...
		{
			sLog.outColor(TNORMAL, LOGONSDROPPHHKATAI, ID);
...
uint32 LogonCommHandler::ClientConnected(string AccountName, WorldSocket * Socket)
{
	...
	DEBUG_LOG(LOGONCOMMCLHANDEAI, SENDINGSDFLHANDEAI, AccountName.c_str(), request_id);
