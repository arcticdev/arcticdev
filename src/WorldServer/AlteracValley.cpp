...
void *AVNode::operator new(size_t size)
{
	sLog.outDebug("New AVNode dynamic string in object.");
	return malloc(size);
}

void AVNode::operator delete(void* p)
{
	sLog.outDebug("Delete AVNode dynamic string in object.");
	free(p);
}

//Конструктор и деструктор для динамических строк в объектах
void *AlteracValley::operator new(size_t size)
{
	sLog.outDebug("New AlteracValley dynamic string in object.");
	return malloc(size);
}

void AlteracValley::operator delete(void* p)
{
	sLog.outDebug("Delete AlteracValley dynamic string in object.");
	free(p);
}
...
{	
	...
	{ STORMPIKEAIDSTATIOAI,		true,	true,	{ 634.607971f, 47.408627f, 69.890488f },
	...
	// name,						GY?		cap?	graveyard location
	{ STORMPIKEGRAVEYARDAI,		true,	true,	{ 675.970947f, -373.707336f, 29.780785f },
	...
	// name,						GY?		cap?	graveyard location
	{ STONEHEARCTHGRAVERAI,		true,	true,	{ 73.869370f, -495.597656f, 48.740143f },
	...
	// name,					GY?		cap?	graveyard location
	{ SNOEFALLGRAVEYARDSAI,		true,	true,	{ -161.472916f, 34.797512f, 77.191841f },
	...
	// name,				GY?		cap?	graveyard location
	{ COLODTOOTHMINESSSSAI,	false,	true,	{ 0.0f, 0.0f, 0.0f },
	...
	// name,					GY?		cap?	graveyard location
	{ IRONDEENMINESSSSSSAI,			false,	true,	{ 0.0f, 0.0f, 0.0f },
	...
	// name,					GY?		cap?	graveyard location
	{ ICEDLOODGRAVEYARDDAI,		true,	true,	{ -531.067627f, -405.459778f, 49.552971f },
	...
	// name,					GY?		cap?	graveyard location
	{ FROSTEOLFGRAVEYARDAI,	true,	true,	{ -1088.008545f, -248.774918f, 57.680843f },
	...
	// name,					GY?		cap?	graveyard location
	{ FROSTEOLFRELIEFHUFAI,	true,	true,	{ -1500.063599f, -333.146393f, 101.133186f },
	...
	// name,					GY?		cap?	graveyard location
	{ NORDHDUNBALDERBUNKAI, false,	false,	{ 0.0f, 0.0f, 0.0f },
	...
	// name,					GY?		cap?	graveyard location
	{ SOUNTSDUNALDERBUNKAI, false,	false,	{ 0.0f, 0.0f, 0.0f },
	...
	// name,			GY?		cap?	graveyard location
	{ ICEWINGBUNKERDDDDDAI,	false,	false,	{ 0.0f, 0.0f, 0.0f },
	...
	// name,				GY?		cap?	graveyard location
	{ STONEHEARCTHBUNKERAI,	false,	false,	{ 0.0f, 0.0f, 0.0f },
	...
	// name,			GY?		cap?	graveyard location
	{ ICEDLOODTOWERDFDFDAI,	false,	false,	{ 0.0f, 0.0f, 0.0f },
	...
	// name,		 GY?	cap?	graveyard location
	{ TOWERPOINTSKFKFSDDAI, false,	false,	{ 0.0f, 0.0f, 0.0f },
	...
	// name,					GY?		cap?	graveyard location
	{ RASTFROSTWOLFTOWERAI,	false,	false,	{ 0.0f, 0.0f, 0.0f },
	...
	// name,					GY?		cap?	graveyard location
	{ WESTFROSTWOLFTOWERAI,	false,	false,	{ 0.0f, 0.0f, 0.0f },
...
AVNode::AVNode( AlteracValleyPointer parent, AVNodeTemplate *tmpl, uint32 nodeid) : m_template(tmpl), m_nodeId(nodeid)
{
	...
	if( !m_template->m_capturable && m_template->m_initialSpawnId )
	{
		...
		CreaturePointer sp;
		OUT_DEBUG(SPAENINGGUARDSATBUAI, m_template->m_name, ci->Name, ci->Id);
...
void AVNode::Assault(PlayerPointer plr)
{
	...
	// safety check
	OUT_DEBUG(AVNOTESUONVNOUIYDFAI, m_template->m_name);
	...
	{
		// send message
		m_bg->SendChatMessage(CHAT_MSG_BG_SYSTEM_ALLIANCE + plr->GetTeam(), 0, ZCLAIMSLEFSUNCHALLAI, plr->GetName(), m_template->m_name,
			plr->GetTeam() ? HORDEZONEAI : ALLIANCESAI);
		
		plr->m_bgScore.MiscData[BG_SCORE_AV_GRAVEYARDS_ASSAULTED]++;
	}
	else
	{
		m_bg->Herald(ISATTASCKWILLDESTTAI, m_template->m_name , plr->GetTeam() ? HORDEZONEAI : ALLIANCESAI);
...
void AVNode::Spawn()
{
	...
	OUT_DEBUG(AVNOVESPAWNSSTATESAI, m_template->m_name, m_state, g_stateNames[m_state], m_lastState, g_stateNames[m_lastState]);
	if( m_template->m_flagLocation.id[m_state] == 0 )
	{
		// no flag should be spawned, despawn if one exists
		if( m_flag != NULL )
		{
			OUT_DEBUG(AVNOTESPAWNSDESPAWAI, m_template->m_name);
			m_flag->Despawn(0);
			m_flag = NULLGOB;
		}
	}
	else
	{
		...
		OUT_DEBUG(AVNONODESSPAVNSSSWAI, m_template->m_name);
		if( m_flag == NULL )
		{
			// initial spawn
	...
	if( m_template->m_auraLocation.id[m_state] == 0 )
	{
		...
		if( m_aura != NULL )
		{
			OUT_DEBUG(AVNOTESECONDARYSSEAI, m_template->m_name);
			m_aura->Despawn(0);
			m_aura = NULLGOB;
		}
	}
	else
	{
		...
		OUT_DEBUG(AVNOVESPAWEWNSSECOAI, m_template->m_name);
		if( m_aura == NULL )
		{
			// initial spawn
	...
	if( m_template->m_glowLocation.id[m_state] == 0 )
	{
		...
		if( m_glow != NULL )
		{
			OUT_DEBUG(AVNOVESPAWNDSESEPIAI, m_template->m_name);
			m_glow->Despawn(0);
			m_glow = NULLGOB;
		}
	}
	else
	{
		...
		OUT_DEBUG(SPAWNINGSGLOWESAVNAI, m_template->m_name);
		if( m_glow == NULL )
		{
			// initial spawn
	...
	{
		OUT_DEBUG(AVNOTESPAWNSGUIARDAI, m_template->m_name);
		...
		{
			OUT_DEBUG(AVNOTESUOUUIOUIYRDAI, m_template->m_name, m_template->m_guardCount, m_template->m_guardId[t]);
...
/*#ifdef COLLISION
	...
	if( m_template->m_isGraveyard )
	{
		...
		{
			OUT_DEBUG(AVNOTESUONVNOUIYRDAI, m_template->m_name);
		...
		{
			OUT_DEBUG(AVNOTESUONVNOUIYVNAI, m_template->m_name);
		...
		{
			OUT_DEBUG(AVNOTESUONVNOSDAVNAI, m_template->m_name);
	...
	OUT_DEBUG(AVNOTESUONVNOSDXVXAI, m_template->m_name, m_state, g_stateNames[m_state]);
}

void AVNode::SpawnGuards(uint32 x)
{
	OUT_DEBUG(AVNOTEJKONVNOSDXVXAI, m_template->m_name, m_template->m_guardCount);
}

void AVNode::ChangeState(uint32 new_state)
{
	OUT_DEBUG(AVNOTEJKONVNOSDASDAI, m_template->m_name, new_state, g_stateNames[new_state], m_state, g_stateNames[m_state]);
	m_lastState = m_state;
	m_state = new_state;
...
void AVNode::Capture()
{
	...
	OUT_DEBUG(AVNOTSDKONVNOSDASDAI, m_template->m_name);
	...
	if( !m_template->m_capturable )
	{
		if( m_state != m_template->m_defaultState )
		{
			...
			OUT_DEBUG(SPAWNSFIRESATBUNKEAI, m_template->m_name);
			while(spi->x != 0.0f)
			{
			...
			// send message
			m_bg->Herald(THEHASDESTROYERSSHAI, (m_template->m_defaultState == AV_NODE_STATE_ALLIANCE_CONTROLLED) ? HORDEZONEAI : ALLIANCESAI, m_template->m_name);
		...
		{
			// saved the tower
			m_bg->Herald(THEHASDTAKERNTAKEEAI, (m_state == AV_NODE_STATE_ALLIANCE_CONTROLLED) ? ALLIANCESAI : HORDEZONEAI, m_template->m_name);
		}
	}
	else
	{
...
void AlteracValley::Herald(const char *format, ...)
{
	...
	data << HEARALIDHEASPRALIDAI;			// Herald
	data << uint64(0);
