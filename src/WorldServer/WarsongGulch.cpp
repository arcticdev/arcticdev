...
void *WarsongGulch::operator new(size_t size)
{
	sLog.outDebug("New WarsongGulch dynamic string in object.");
	return malloc(size);
}

void WarsongGulch::operator delete(void* p)
{
	sLog.outDebug("Delete WarsongGulch dynamic string in object.");
	free(p);
}
...
void WarsongGulch::HookOnAreaTrigger(PlayerPointer plr, uint32 id)
{
	...
	if(buffslot > -1)
	{
		if(m_buffs[buffslot] != 0 && m_buffs[buffslot]->IsInWorld())
		{
			...
			SpellPointer s(new Spell(plr, sp, true, NULLAURA));
...
void WarsongGulch::HookFlagDrop(PlayerPointer plr, GameObjectPointer obj)
{
	...
	SpellPointer sp(new Spell(plr, pSp, true, NULLAURA));
	SpellCastTargets targets(plr->GetGUID());
	sp->prepare(&targets);
