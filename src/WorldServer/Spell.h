...
enum procFlags
{
   ...
   PROC_ON_MELEE_ATTACK_VICTIM = 0x8,
	PROC_ON_MELEE_SPELL = 0x10,
	PROC_ON_MELEE_SPELL_VICTIM = 0x20,

   PROC_ON_RANGED_ATTACK = 0x40,
	PROC_ON_RANGED_ATTACK_VICTIM = 0x80,
	PROC_ON_RANGED_SPELL = 0x100,
	PROC_ON_RANGED_SPELL_VICTIM = 0x200,

	PROC_ON_ADD_AURA	= 0x400,
	PROC_ON_ADD_AURA_VICTIM = 0x800,
   PROC_ON_CAST_SPELL = 0x1000,
	PROC_ON_AURA_REMOVE	= 0x2000,

   PROC_ON_POS_SPELL_LAND	= 0x4000,
   PROC_ON_POS_SPELL_LAND_VICTIM = 0x8000,
   PROC_ON_NEG_SPELL_LAND	= 0x10000,
	PROC_ON_NEG_SPELL_LAND_VICTIM = 0x20000,

   PROC_ON_DOT_TICK	= 0x40000,
   PROC_ON_DOT_TICK_VICTIM = 0x80000,

	// free = 0x100000,
	// free = 0x200000,

	PROC_ON_TRAP_TRIGGER	= 0x400000,
	PROC_ON_OFFHAND_ATTACK = 0x800000,

   PROC_REMOVEONUSE = 0x4000000, // remove prochcharge only when it is used
   PROC_MISC = 0x8000000,
	PROC_ON_TARGET_DIE = 0x10000000,
	PROC_TARGET_SELF	= 0x20000000, // flag to decide if proc target is self or victim

	PROC_ON_PHYSICAL_ATTACK_VICTIM = ( PROC_ON_MELEE_ATTACK_VICTIM | PROC_ON_MELEE_SPELL_VICTIM |
      PROC_ON_RANGED_ATTACK_VICTIM | PROC_ON_RANGED_SPELL_VICTIM ),

   PROC_ON_PHYSICAL_ATTACK = ( PROC_ON_MELEE_ATTACK | PROC_ON_MELEE_SPELL |
      PROC_ON_RANGED_ATTACK | PROC_ON_RANGED_SPELL ),	

	PROC_SPELL_ACTION = (PROC_ON_MELEE_SPELL | PROC_ON_RANGED_SPELL | PROC_ON_ADD_AURA |
      PROC_ON_CAST_SPELL | PROC_ON_POS_SPELL_LAND | PROC_ON_NEG_SPELL_LAND |
		PROC_ON_DOT_TICK | PROC_ON_TRAP_TRIGGER),

	PROC_ON_ANY_DAMAGE = (PROC_ON_PHYSICAL_ATTACK | PROC_ON_NEG_SPELL_LAND ),

	PROC_ON_ANY_DAMAGE_VICTIM = (PROC_ON_PHYSICAL_ATTACK_VICTIM | PROC_ON_NEG_SPELL_LAND_VICTIM ),
};

enum ExtraProcFlags
{
   EXTRA_NONE = 0x0000000,
   EXTRA_ON_CRIT = 0x0000001,
   EXTRA_ON_MISS = 0x0000002,
   EXTRA_ON_PARRY = 0x0000004,
   EXTRA_ON_DODGE = 0x0000008,
   EXTRA_ON_BLOCK = 0x0000010,
   EXTRA_ON_RESIST = 0x0000020,
   EXTRA_ON_ABSORB = 0x0000040,
   EXTRA_ON_REFLECT = 0x0000080,
   EXTRA_ON_DISPEL = 0x0000100,
   EXTRA_ON_EXPIRE = 0x0000200,
   EXTRA_ON_SUCCESHIT = 0x0000400,
   EXTRA_ON_ENERGIZE = 0x0000800,
   EXTRA_ON_HEAL = 0x0001000,
   EXTRA_ON_DAMAGE = 0x0002000,
   EXTRA_ON_DIE = 0x0004000,
};
...
enum Attributes
{
   ATTRIBUTES_NULL = 0x0,
	...
	ATTRIBUTES_TRIGGER_COOLDOWN = 0x2000000,
...
ARCTIC_INLINE bool TargetTypeCheck(Object* obj,uint32 ReqCreatureTypeMask)
{
	...
	else if( obj -> GetTypeId() == TYPEID_PLAYER && !( UNIT_TYPE_HUMANOID/*_BIT*/ & ReqCreatureTypeMask ) )
		return false;
    ...
    uint32 GetDuration()
    {
        ...
        if(m_spellInfo->DurationIndex)
        {
            ...
            if(sd)
            {
				...
				if( unitTarget && p_caster && unitTarget == p_caster )
				{
