...
void *BasicTaskExecutor::operator new(size_t size)
{
	sLog.outDebug("New BasicTaskExecutor dynamic string in object.");
	return malloc(size);
}

void BasicTaskExecutor::operator delete(void* p)
{
	sLog.outDebug("Delete BasicTaskExecutor dynamic string in object.");
	free(p);
}
...
{
	sLog.outDebug("New Task dynamic string in object.");
	return malloc(size);
}

void Task::operator delete(void* p)
{
	sLog.outDebug("Delete Task dynamic string in object.");
	free(p);
}

//Конструктор и деструктор для динамических строк в объектах
void *CharacterLoaderThread::operator new(size_t size)
{
	sLog.outDebug("New CharacterLoaderThread dynamic string in object.");
	return malloc(size);
}

void CharacterLoaderThread::operator delete(void* p)
{
	sLog.outDebug("Delete CharacterLoaderThread dynamic string in object.");
	free(p);
}

//Конструктор и деструктор для динамических строк в объектах
void *TaskExecutor::operator new(size_t size)
{
	sLog.outDebug("New TaskExecutor dynamic string in object.");
	return malloc(size);
}

void TaskExecutor::operator delete(void* p)
{
	sLog.outDebug("Delete TaskExecutor dynamic string in object.");
	free(p);
}
...
//Конструктор и деструктор для динамических строк в объектах
void *World::operator new(size_t size)
{
	sLog.outDebug("New World dynamic string in object.");
	return malloc(size);
}

void World::operator delete(void* p)
{
	sLog.outDebug("Delete World dynamic string in object.");
	free(p);
}
...
{
	Log.Notice("WoWArcTic", "I'm shutting down!");
...
// ------------------------------------------------------------------------------------------------
	...
	Log.Notice("World", "Starting DayWatcherThread...");
...
void World::Rehash(bool load)
{
	...
	switch(Config.MainConfig.GetIntDefault( "Server", "AdjustPriority", 0 ))
	{
		case 0:
			{
		...
		case -1:
			{
				if( current_priority_class != IDLE_PRIORITY_CLASS )
