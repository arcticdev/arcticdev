...
void *WorldStateManager::operator new(size_t size)
{
	sLog.outDebug("New WorldStateManager dynamic string in object.");
	return malloc(size);
}

void WorldStateManager::operator delete(void* p)
{
	sLog.outDebug("Delete WorldStateManager dynamic string in object.");
	free(p);
}
