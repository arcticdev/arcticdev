...
{
	PlayerPointer SelectedPlayer = getSelectedChar(m_session, true);
...
bool ChatHandler::HandleBanCharacterCommand(const char* args, WorldSession *m_session)
{
	...
	*pReason = 0;
	...
	PlayerPointer pPlayer = objmgr.GetPlayer(pCharacter, false);
	if(pPlayer == NULL)
	{
...
bool ChatHandler::HandleUnBanCharacterCommand(const char* args, WorldSession *m_session)
{
	...
	PlayerPointer pPlayer = ObjectMgr::getSingleton( ).GetPlayer(Character, false);
	if(pPlayer != 0)
	{
...
bool ChatHandler::HandleGMTicketGetAllCommand(const char* args, WorldSession *m_session)
{
	...
	for(GmTicketList::iterator itr = objmgr.GM_TicketList.begin(); itr != objmgr.GM_TicketList.end(); itr++)
	{
		uint32 cont = 0;
		uint32 zone = 0;
		PlayerPointer plr = objmgr.GetPlayer((uint32)(*itr)->guid);
		/*if(plr)
		{
			if(plr->IsInWorld())
			{
...
bool ChatHandler::HandleGMTicketDelByIdCommand(const char* args, WorldSession *m_session)
{
	...
	if(guid != -1)
	{
		...
		chn->Say(m_session->GetPlayer(), str.str().c_str(), NULLPLR, true);


		PlayerPointer plr = objmgr.GetPlayer((uint32)guid);
		if(!plr)
			return true;
		if(!plr->IsInWorld())
			return true;
...
{
	char buf[256];
	PlayerPointer target = objmgr.GetPlayer((uint32)m_session->GetPlayer()->GetSelection());
...
{
	PlayerPointer plyr = getSelectedChar(m_session, true);
	if(!plyr) return true;
...
bool ChatHandler::HandlePetSpawnAIBot(const char* args, WorldSession *m_session)
{
	...
	CreaturePointer pCreature = new Creature(HIGHGUID_UNIT ,1);//no need in guid
...
bool ChatHandler::HandleMassSummonCommand(const char* args, WorldSession* m_session)
{
	...
	PlayerPointer summoner = m_session->GetPlayer();
	PlayerPointer plr;
	uint32 c=0;
...
{
	CreaturePointer creature = getSelectedCreature(m_session);
...
bool ChatHandler::HandleFormationLink2Command(const char* args, WorldSession * m_session)
{
	...
	CreaturePointer slave = getSelectedCreature(m_session, true);
	...
	BlueSystemMessage(m_session, "%s linked up to %s with a distance of %f at %f radians.", slave->GetCreatureName()->Name, 
		TO_CREATURE( m_session->GetPlayer()->linkTarget )->GetCreatureName()->Name, dist, ang );
...
{
	CreaturePointer creature = getSelectedCreature(m_session, true);
...
{
	CreaturePointer c = getSelectedCreature(m_session, true);
...
bool ChatHandler::HandleForceRenameCommand(const char * args, WorldSession * m_session)
{
	...
	PlayerPointer plr = objmgr.GetPlayer((uint32)pi->guid);
	if(plr == 0)
	{
...
{
	UnitPointer pTarget = getSelectedChar(m_session, false);
	if(!pTarget)
	{
...
SpellCastTargets SetTargets(SpellEntry * sp, uint32 type, uint32 targettype, UnitPointer dst, CreaturePointer src)
{
...
bool ChatHandler::HandleCollisionTestIndoor(const char * args, WorldSession * m_session)
{
	...
	{
		SystemMessage(m_session, "WoWArcTic was not compiled with collision support.");
		return true;
	}
}
...
bool ChatHandler::HandleCollisionTestLOS(const char * args, WorldSession * m_session)
{
	if (sWorld.Collision)
	{
		ObjectPointer pObj = NULLOBJ;
		CreaturePointer pCreature = getSelectedCreature(m_session, false);
		PlayerPointer pPlayer = getSelectedChar(m_session, false);
		if(pCreature)
			pObj = pCreature;
		else if(pPlayer)
			pObj = pPlayer;
...
bool ChatHandler::HandleDebugSpellWarCommand(const char *args, WorldSession *m_session)
{
	CreaturePointer pCreature = m_session->GetPlayer()->GetMapMgr()->GetInterface()->SpawnCreature(4949,
		m_session->GetPlayer()->GetPositionX() - 1.0f, m_session->GetPlayer()->GetPositionY() - 1.0f, m_session->GetPlayer()->GetPositionZ(), 0.0f, true, true, 0, 0);

	CreaturePointer pCreature2 = m_session->GetPlayer()->GetMapMgr()->GetInterface()->SpawnCreature(4949,
		m_session->GetPlayer()->GetPositionX() + 1.0f, m_session->GetPlayer()->GetPositionY() + 1.0f, m_session->GetPlayer()->GetPositionZ(), 0.0f, true, true, 0, 0);

	pCreature->SetUInt32Value(UNIT_FIELD_MAXHEALTH, 0x7FFFFFFF);
	pCreature->SetUInt32Value(UNIT_FIELD_HEALTH, 0x7FFFFFFF);
	pCreature2->SetUInt32Value(UNIT_FIELD_MAXHEALTH, 0x7FFFFFFF);
	pCreature2->SetUInt32Value(UNIT_FIELD_HEALTH, 0x7FFFFFFF);

	pCreature->SetUInt32Value(UNIT_FIELD_FACTIONTEMPLATE, 14);
	pCreature->_setFaction();

	pCreature->GetMapMgr()->AddForcedCell( pCreature->GetMapCell() );

	SpellEntry* sp;
	for( uint32 i = 0; i < dbcSpell.GetNumRows(); ++i)
	{
		sp = dbcSpell.LookupRow(i);
		
		sEventMgr.AddEvent( TO_UNIT(pCreature), &Unit::EventCastSpell, TO_UNIT(pCreature2), (SpellEntry*)sp, EVENT_UNIT_CHAT_MSG, i * 100, 0, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
		sEventMgr.AddEvent( TO_UNIT(pCreature), &Unit::SetHealthPct, uint32(100), EVENT_UNK, 100 * i, 0, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
		sEventMgr.AddEvent( TO_UNIT(pCreature2), &Unit::SetHealthPct, uint32(100), EVENT_UNK, 100 * i, 0, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	}
	return true;
}
