...
void *TerrainMgr::operator new(size_t size)
{
	sLog.outDebug("New TerrainMgr dynamic string in object.");
	return malloc(size);
}

void TerrainMgr::operator delete(void* p)
{
	sLog.outDebug("Delete TerrainMgr dynamic string in object.");
	free(p);
}
