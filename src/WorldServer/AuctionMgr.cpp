...
void *AuctionMgr::operator new(size_t size)
{
	sLog.outDebug("New AuctionMgr dynamic string in object.");
	return malloc(size);
}

void AuctionMgr::operator delete(void* p)
{
	sLog.outDebug("Delete AuctionMgr dynamic string in object.");
	free(p);
}
