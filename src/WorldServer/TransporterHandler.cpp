...
void *Transporter::operator new(size_t size)
{
	sLog.outDebug("New Transporter dynamic string in object.");
	return malloc(size);
}

void Transporter::operator delete(void* p)
{
	sLog.outDebug("Delete Transporter dynamic string in object.");
	free(p);
}
