...
void *Creature::operator new(size_t size)
{
	sLog.outDebug("New Creature dynamic string in object.");
	return malloc(size);
}

void Creature::operator delete(void* p)
{
	sLog.outDebug("Delete Creature dynamic string in object.");
	free(p);
}
...
void Creature::RemoveFromWorld(bool addrespawnevent, bool free_guid)
{
	...
	if(IsPet()) // Is a pet: IsPet() actually returns false on a pet? 
	{
		if(IsInWorld())
...
//////////////////////////////////////////////////////////////
	...
	//BLooD_LorD:blizzlike code for humanoids run away if they have health lower then 10% of maximum value
	if(creature_info->Family = HUMANOID)
	{
...
void Creature::Load(CreatureProto * proto_, float x, float y, float z, float o)
{
    ...
    else if( proto->GuardType == 2 )
        m_aiInterface->m_isNeutralGuard = true;
    else if( proto->GuardType == 3 )
    {
        m_aiInterface->m_isGuard = true;
        m_aiInterface->m_isNeutralGuard = true;
    }
