...
void CBattlegroundManager::HandleBattlegroundJoin(WorldSession * m_session, WorldPacket & pck)
{
	...
	if( joinasgroup && m_session->GetPlayer()->GetGroup())
	{
		...
		Log.Success("BattlegroundManager", "Group %u is now in battleground queue for instance %u", m_session->GetPlayer()->GetGroup(), instance );
	}
	else
	...
	for(i = 0; i < BATTLEGROUND_NUM_TYPES; ++i)
	{
		for(j = 0; j < MAX_LEVEL_GROUP; ++j)
		{
			...
			for(it3 = m_queuedPlayers[i][j].begin(); it3 != m_queuedPlayers[i][j].end();)
			{
				...
				if(queueSlot >= NUM_BG_QUEUE_SLOTS) continue;
...
void CBattleground::BuildPvPUpdateDataPacket(WorldPacket * data)
{
    ...
    *data << uint64( 0 ); // schnek: Arena team scores for 3.2..
	*data << uint8( m_ended );
	if(m_ended)
	{
...
void CBattleground::AddPlayer(PlayerPointer plr, uint32 team)
{
	...
	if(queueSlot >= NUM_BG_QUEUE_SLOTS)
	{
		m_mainLock.Release();
		return;
	}
...
void CBattlegroundManager::HandleArenaJoin(WorldSession * m_session, uint32 BattlegroundType, uint8 as_group, uint8 rated_match)
{
	...
	if(as_group)
	{
		...
		if(!rated_match)
		{
			...
			{
				if((*itx)->m_loggedInPlayer && ((*itx)->m_loggedInPlayer->m_bgIsQueued[0] ||(*itx)->m_loggedInPlayer->m_bgIsQueued[1]))
					isQueued = true;
