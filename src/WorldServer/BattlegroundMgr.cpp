...
void *CBattlegroundManager::operator new(size_t size)
{
	sLog.outDebug("New CBattlegroundManager dynamic string in object.");
	return malloc(size);
}

void CBattlegroundManager::operator delete(void* p)
{
	sLog.outDebug("Delete CBattlegroundManager dynamic string in object.");
	free(p);
}
...
void CBattlegroundManager::HandleBattlegroundJoin(WorldSession * m_session, WorldPacket & pck)
{
	...
	{
		m_session->GetPlayer()->GetSession()->SendNotification(OUHAVEDEESMARAI);
		return;
	}
	...
	if( joinasgroup && m_session->GetPlayer()->GetGroup() == NULL ) 
	{
		...
		sChatHandler.RedSystemMessage( m_session, YOUARENOTINGRAI );
		return;
	}
	...
	if(instance)
	{
		...
		{
			sChatHandler.SystemMessage(m_session, YOUHAVETRIEDSAI);
			m_instanceLock.Release();
			return;
		}
	...
	{
		plr->BroadcastMessage(ZOSUSSDTRIEDSAI);
		m_queueLock.Release();
		return;
	}
	...
	if( joinasgroup )
	{
		...
		Log.Success(BATTLELEGROMSAI, GROUPNOWBATLLAI, m_session->GetPlayer()->GetGroup()->GetID(), instance );
	}
	else
		Log.Success(BATTLELEGROMSAI, PLAERSTZALAUEAI, m_session->GetPlayer()->GetLowGUID(), instance );
	...
	{
		OUT_DEBUG(WARNINGWOWARCAI, __FUNCTION__);
		m_queueLock.Release();
		return;
	}
...
int CBattlegroundManager::CreateArenaType(int type, Group * group1, Group * group2)
{
	...
	if (ar == NULL)
	{
		Log.Error(ZABATTLESGROUAI, AREAZSACOULDSAI, __FILE__, __LINE__);
		m_queueLock.Release();
		m_instanceLock.Release();
		return -1;
	}
...
void CBattlegroundManager::EventQueueUpdate(bool forceStart)
{
	...
	{
		if(!forceStart && m_queuedGroups[i].size() < 2)      // got enough to have an arena battle.
		{
			continue;
		}
		...
		for (j=0; j<(uint32)m_queuedGroups[i].size(); j++)
		{
			...
			if(itz == m_queuedGroups[i].end())
			{
				Log.Error(ZABATTLESGROUAI, INTERNETSERROAI, __FILE__, __LINE__);
				m_queueLock.Release();
				m_instanceLock.Release();
				return;
			}
...
void CBattlegroundManager::RemovePlayerFromQueues(PlayerPointer plr)
{
	...
	for(i = 0; i < 3; ++i)
	{
		...
		while(itr != m_queuedPlayers[plr->m_bgQueueType[i]][lgroup].end())
		{
			...
			{
				OUT_DEBUG(BATTLELEGROMSAI, REMOSPLAEYRSFAI, plr->GetLowGUID(), plr->m_bgQueueInstanceId[i], plr->m_bgQueueType[i]);
...
void CBattleground::PortPlayer(PlayerPointer plr, bool skip_teleport /* = false*/)
{
	...
	if(m_ended)
	{
		...
		sChatHandler.SystemMessage(plr->GetSession(), YOUSRAJOERTSFAI);
	...
	if( !plr->m_isGmInvisible )
	{
		...
		data << plr->GetGUID();
		for(set<PlayerPointer>::iterator itr = m_players[plr->m_bgTeam].begin(); itr != m_players[plr->m_bgTeam].end(); ++itr)
...
BattlegroundPointer CBattlegroundManager::CreateInstance(uint32 Type, uint32 LevelGroup)
{
	...
	if(Type == BATTLEGROUND_ARENA_2V2 || Type == BATTLEGROUND_ARENA_3V3 || Type == BATTLEGROUND_ARENA_5V5)
	{
		...
		if(mgr == NULL)
		{
			Log.Error(BATTLELEGROMSAI, ZAREANACREANTAI, mapid, Type, LevelGroup);
		...
		Log.Success(BATTLELEGROMSAI, WOWCREATYRESDAI, Type, LevelGroup, mapid);
	...
	if(cfunc == NULL)
	{
		Log.Error(BATTLELEGROMSAI, WOWCOULDSNOTSAI, Type, LevelGroup);
	...
	if(mgr == NULL)
	{
		Log.Error(BATTLELEGROMSAI, CREATURESINSTAI, BGMapIds[Type], Type, LevelGroup);
		return NULLBATTLEGROUND;		// Shouldn't happen
	}
	...
	mgr->m_battleground = bg;
	sEventMgr.AddEvent(bg, &CBattleground::EventCreate, EVENT_BATTLEGROUNDMGR_QUEUE_UPDATE, 1, 1,0);
	Log.Success(BATTLELEGROMSAI, WCREATYRESBATAI, Type, LevelGroup);
...
void CBattlegroundManager::DeleteBattleground(BattlegroundPointer bg)
{
	...
	for(; itr != m_queuedPlayers[i][j].end();)
	{
		...
		for(uint32 z= 0; z < 3; ++z)
		{
			...
			{
				sChatHandler.SystemMessageToPlr(plr, YOURADFJSDGONAI, bg->GetId());
...
void CBattleground::EventCountdown()
{
	...
	{
		m_countdownStage = 2;
		SendChatMessage( CHAT_MSG_BG_SYSTEM_NEUTRAL, 0, ZONESAMINUTESAI, GetName() );
	}
	else if(m_countdownStage == 2)
	{
		m_countdownStage = 3;
		SendChatMessage( CHAT_MSG_BG_SYSTEM_NEUTRAL, 0, ZTRHIRHESECONAI, GetName() );
		sEventMgr.ModifyEventTimeAndTimeLeft(shared_from_this(), EVENT_BATTLEGROUND_COUNTDOWN, 15000);
	}
	else if(m_countdownStage == 3)
	{
		m_countdownStage = 4;
		SendChatMessage( CHAT_MSG_BG_SYSTEM_NEUTRAL, 0, FIFTEENSECONDAI, GetName() );
	}
	else
	{
		SendChatMessage( CHAT_MSG_BG_SYSTEM_NEUTRAL, 0, THEBATLEFORSDAI, GetName() );
...
void CBattlegroundManager::HandleArenaJoin(WorldSession * m_session, uint32 BattlegroundType, uint8 as_group, uint8 rated_match)
{
	...
	if(as_group)
	{
		...
		{
			m_session->SystemMessage(SORRYRAIDSGROAI);
			return;
		}
		if(pGroup->GetLeader() != plr->m_playerInfo)
		{
			m_session->SystemMessage(YUORSMISTSBESAI);
			return;
		}
		...
		else
		{
			...
			if(!pTargetTeam)
			{
				m_session->SystemMessage(YUORSGGGHSBESAI);
				return;
			}
			...
			for(itx = pGroup->GetSubGroup(0)->GetGroupMembersBegin(); itx != pGroup->GetSubGroup(0)->GetGroupMembersEnd(); ++itx)
			{
				if(maxplayers==0)
				{
					m_session->SystemMessage(YOUHATERSMANYAI);
					pGroup->Unlock();
					return;
				}
				...
				if((*itx)->lastLevel < 80)
				{
					m_session->SystemMessage(SORRYSOMESOFIAI);
					pGroup->Unlock();
					return;
				}
				...
				if((*itx)->m_loggedInPlayer)
				{
                   ...
                   { 
                          m_session->SystemMessage(DAZONERSMORESAI); 
	                      pGroup->Unlock(); 
                          return; 
	                } 
					...
					{
						m_session->SystemMessage(OAEZONERSMOREAI);
						pGroup->Unlock();
						return;
					}
				...
				{
					m_session->SystemMessage(ONESORMORESOFAI);
					pGroup->Unlock();
					return;
				}
			}
			...
			m_queueLock.Release();
			Log.Success(ZABATTLESGROUAI, REMOSHGAEYRSFAI, pGroup->GetID(), BattlegroundType);
	...
	Log.Success(ZABATTLESGROUAI, ZAPLAYERSTNOWAI, plr->GetLowGUID(), BattlegroundType );
