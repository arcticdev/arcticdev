...
ARCTIC_DECL CCollideInterface CollideInterface;
...
bool CCollideInterface::CheckLOS(uint32 mapId, float x1, float y1, float z1, float x2, float y2, float z2)
{
	...
	bool res = false;
	// get data
	if(CollisionMgr->isInLineOfSight(mapId, x1, y1, z1, x2, y2, z2))
		res = true;

	if(!res && CollisionMgr->isInLineOfSight(mapId, x1, y1, z1+3.0f , x2, y2, z2+3.0f ))
		res = true;
