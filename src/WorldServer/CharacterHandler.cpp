...
bool ChatHandler::HandleRenameAllCharacter(const char * args, WorldSession * m_session)
{
		...
		do
		{
			...
			if( !VerifyName(pName, szLen) )
			{
				...
				if( pPlayer != NULL )
				{
					...
					pPlayer->GetSession()->SystemMessage(ZCHARACTERASAI);
				}
	...
	SystemMessage(m_session, PROCEDURESCOAI, getMSTime() - ts, uCount);
	return true;
}
...
uint8 WorldSession::DeleteCharacter(uint32 guid)
{
	...
	if( inf != NULL && inf->m_loggedInPlayer == NULL )
	{
		...
		sPlrLog.writefromsession(this, ZDELETESDACHAI, name.c_str(), (uint32)guid);
...
void WorldSession::HandleCharRenameOpcode(WorldPacket & recv_data)
{
	...
	sPlrLog.writefromsession(this, ARENAMEWSADSAI, pi->name, pi->guid, name.c_str());
...
void WorldSession::HandlePlayerLoginOpcode( WorldPacket & recv_data )
{
	...
	DEBUG_LOG( ZWORLDSESSIOAI, REVESATWPLAYSI );
	...
	DEBUG_LOG(ZWORLDSESSIOAI, ASYERSTEIAMSAI, (uint32)playerGuid);
...
{
	DEBUG_LOG(ZWORLDSESSIOAI, FULLYAKILLSDAI, plr->GetLowGUID());
	SetPlayer(plr);
	...
	*/
	...
	DEBUG_LOG( ZWORLDSESSIOAI,PLAEYRSLOGGSAI, plr->GetName());
	...
	DEBUG_LOG( ZWORLDSESSIOAI,GREATYSLOGGSAI, plr->GetName() );
...
#ifdef WIN32
	_player->BroadcastMessage(SERVERWOWARCTICWIN, MSG_COLOR_WHITE, BUILD_REVISION, CONFIG, ARCH, MSG_COLOR_LIGHTBLUE);
#else
	_player->BroadcastMessage(SERVERWOWARCTICS, MSG_COLOR_WHITE, BUILD_REVISION, PLATFORM_TEXT, ARCH, MSG_COLOR_LIGHTBLUE);

#endif

	if(sWorld.SendStatsOnJoin)
	{
		_player->BroadcastMessage(ONLIINEPLAYERSARCTIC,
		...
		_player->BroadcastMessage(SERVERZUPTIMESARCTIC, sWorld.GetUptimeString().c_str());
	}

	// send to gms
	if( HasGMPermissions() )
		sWorld.SendMessageToGMs(this, GMNOWONLINENOWAI, _player->GetName(), GetAccountNameS(), GetPermissions());
...
bool ChatHandler::HandleRenameCommand(const char * args, WorldSession * m_session)
{
	...
	{
		RedSystemMessage(m_session, THATNAMESISIAI);
		return true;
	}
	...
	if(pi == 0)
	{
		RedSystemMessage(m_session, PLAEYNOTFOUNAI);
		return true;
	}

	if( objmgr.GetPlayerInfoByName(new_name.c_str()) != NULL )
	{
		RedSystemMessage(m_session, PLAYERSFOURDAI);
		return true;
	}
	...
	if(plr != 0)
	{
		...
		BlueSystemMessageToPlr(plr, CHARACTERTYOAI, m_session->GetPlayer()->GetName(), new_name.c_str());
		plr->SaveToDB(false);
	}
	else
	{
	...
	GreenSystemMessage(m_session, CHANDERNAMESAI, name1, name2);
	sGMLog.writefromsession(m_session, RENAMEMEDCHAAI, name1, pi->guid, name2);
	sPlrLog.writefromsession(m_session, ZGMCHARACTERAI, name1, pi->guid, name2);
	return true;
}
...
{ 
    ...
    //if(!objmgr.GetPlayerNameByGUID(name, declinedname[0]))
        //error = 1; 
