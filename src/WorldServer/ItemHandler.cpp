...
void WorldSession::HandleBuyItemInSlotOpcode( WorldPacket & recv_data ) // drag & drop
{
	...
	recv_data >> vendorslot; // 3.1.2 This is the slot's number on the vendor's panel, starts from 1
	recv_data >> bagguid;
	recv_data >> slot; // 3.1.2 the target slot the player selected - backpack 23-38, other bags 0-15 (Or how big is the biggest bag? 0-127?)
	recv_data >> slot;
...
void WorldSession::HandleRepairItemOpcode(WorldPacket &recvPacket)
{
	...
	if( !pCreature->HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_REPAIR ) )
		return;
