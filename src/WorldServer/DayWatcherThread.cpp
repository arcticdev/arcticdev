...
void *DayWatcherThread::operator new(size_t size)
{
	sLog.outDebug("New DayWatcherThread dynamic string in object.");
	return malloc(size);
}

void DayWatcherThread::operator delete(void* p)
{
	sLog.outDebug("Delete DayWatcherThread dynamic string in object.");
	free(p);
}
...
void DayWatcherThread::load_settings()
{
	...
	result = CharacterDatabase.Query("SELECT setting_value FROM server_settings WHERE setting_id = \"last_premium_update_time\"");
	if(result)
	{
		last_premium_update_time = result->Fetch()[0].GetUInt32();
		delete result;
	}
	else
	{
