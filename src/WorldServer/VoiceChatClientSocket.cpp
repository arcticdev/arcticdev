...
void *VoiceChatClientSocket::operator new(size_t size)
{
	sLog.outDebug("New VoiceChatClientSocket dynamic string in object.");
	return malloc(size);
}

void VoiceChatClientSocket::operator delete(void* p)
{
	sLog.outDebug("Delete VoiceChatClientSocket dynamic string in object.");
	free(p);
}
