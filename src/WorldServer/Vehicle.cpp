...
void *Vehicle::operator new(size_t size)
{
	sLog.outDebug("New Vehicle dynamic string in object.");
	return malloc(size);
}

void Vehicle::operator delete(void* p)
{
	sLog.outDebug("Delete Vehicle dynamic string in object.");
	free(p);
}
...
void Vehicle::_AddToSlot(UnitPointer pPassenger, uint8 slot)
{
	...
	if( pPassenger->IsPlayer() )
	{
		...
		if (pPlayer->GetSession() != NULL)
			pPlayer->GetSession()->SendPacket(&ack);

		WorldPacket data1(SMSG_ON_CANCEL_EXPECTED_RIDE_VEHICLE_AURA, 0);
		...
		WorldPacket data;
		data.Initialize(MSG_MOVE_TELEPORT);
