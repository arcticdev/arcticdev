...
{
	if( res->GetFieldCount() != CREATURESPAWNSFIEL )
	{
		if( first_table_warning )
		{
			first_table_warning = false;
			Log.LargeErrorMessage(LARGERRORMESSAGE_WARNING, "One of your creature_spawns table has the wrong column count.", // schnek: creature_spawns
				"WoWArcTic has skipped loading this table in order to avoid crashing.",
...
bool CheckResultLengthGameObject(QueryResult * res)
{
	if( res->GetFieldCount() != GOSPAWNSFIELDCOUNT )
	{
		if( first_table_warningg )
		{
			...
			Log.LargeErrorMessage(LARGERRORMESSAGE_WARNING, "One of your gameobject_spawns table has the wrong column count.", // schnek: gameobject_spawns
				"WoWArcTic has skipped loading this table in order to avoid crashing.",
				"Please correct this, if you do not no spawns will show.", NULL);
		}
...
void Map::LoadSpawns(bool reload)
{
	...
	if ( reload ) // schnek: выполнить очистку
		for ( uint32 x = 0; x < _sizeX; x++ )
			for ( uint32 y = 0; y < _sizeY; y++ )
				if ( spawns[x][y])
				{	
