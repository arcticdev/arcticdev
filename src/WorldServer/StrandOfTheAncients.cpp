...
void *StrandOfTheAncients::operator new(size_t size)
{
	sLog.outDebug("New StrandOfTheAncients dynamic string in object.");
	return malloc(size);
}

void StrandOfTheAncients::operator delete(void* p)
{
	sLog.outDebug("Delete StrandOfTheAncients dynamic string in object.");
	free(p);
}
