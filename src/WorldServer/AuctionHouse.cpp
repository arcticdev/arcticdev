...
void *AuctionHouse::operator new(size_t size)
{
	sLog.outDebug("New AuctionHouse dynamic string in object.");
	return malloc(size);
}

void AuctionHouse::operator delete(void* p)
{
	sLog.outDebug("Delete AuctionHouse dynamic string in object.");
	free(p);
}
	...
	case AUCTION_REMOVE_CANCELLED:
		{
			...
			{
				sMailSystem.DeliverMessage(AUCTION, GetID(), auct->HighestBidder, subject, "", auct->HighestBid,  0, 0, 62,true); 
			}
