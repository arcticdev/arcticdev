...
void AIInterface::Init(Unit* un, AIType at, MovementType mt)
{
	...
	m_runSpeed = GetUnit()->m_runSpeed *0.001f;   // move distance per ms time 
...
void AIInterface::Update(uint32 p_time)
{
	...
	if(getAIType() == AITYPE_TOTEM)
	{
		...
		if(p_time >= m_totemspelltimer)
		{
			...
			pSpell = new Spell(GetUnit(), totemspell, true, NULLAURA);
							...
							if(GetNextTarget() != NULL)
							{
										...
										&& Rand(GetUnit()->CalculateDazeCastChance(GetNextTarget())))
								{
									...
									if(fabs(our_facing-his_facing)<CREATURE_DAZE_TRIGGER_ANGLE && !GetNextTarget()->HasNegativeAura(CREATURE_SPELL_TO_DAZE))
									{
										...
										Spell* sp = NULLSPELL;
										sp = new Spell(GetUnit(), info, false, NULLAURA);
		...
		case AGENT_SPELL:
			{
				...
				if(los && ((distance <= (m_nextSpell->maxrange*m_nextSpell->maxrange)  && distance >= (m_nextSpell->minrange*m_nextSpell->minrange)) || m_nextSpell->maxrange == 0)) // Target is in Range -> Attack
				{
					...
					if( ccr == SPELL_CANCAST_OK )
					{
						...
						DEBUG_LOG("AiAgents","Spell failed: Result %u, NPC %u, spell %u, TargetType %u", ccr, GetUnit()->GetEntry() , spellInfo->Id, targettype );
				}
		...
		case AGENT_FLEE:
			{
				...
				data << TO_CREATURE( GetUnit() )->GetCreatureInfo()->Name;
				data << uint64( 0 );
...
{// find nearest hostile Target to attack
	...
	float dist;
	bool pvp = true;
	if(GetUnit()->GetTypeId()==TYPEID_UNIT&&TO_CREATURE(GetUnit())->GetCreatureInfo()&&TO_CREATURE(GetUnit())->GetCreatureInfo()->Civilian)
		pvp=false;
	...
	if( target )
	{
		...
		{
			Unit* target2 = GetUnit()->GetMapMgr()->GetPlayer(target->GetUInt32Value(UNIT_FIELD_CREATEDBY));
			if(target2)
			{
...
bool AIInterface::FindFriends(float dist)
{
	...
	CreatureInfo * ci = TO_CREATURE(GetUnit())->GetCreatureInfo();
	if( ci && ci->Type == UNIT_TYPE_HUMANOID && ci->Civilian )
		CallGuards();	
...
float AIInterface::_CalcAggroRange(Unit* target)
{
	...
	if(TO_CREATURE(GetUnit())->GetCreatureInfo() && TO_CREATURE(GetUnit())->GetCreatureInfo()->Rank > 0)
		AggroRange *= (TO_CREATURE(GetUnit())->GetCreatureInfo()->Rank) * 1.50f;
...
void AIInterface::UpdateMove()
{
	...
	if(GetUnit()->GetTypeId() == TYPEID_UNIT)
	{
		Creature* creature = TO_CREATURE(GetUnit());
	...
	else if(m_creatureState == STOPPED && (getAIState() == STATE_IDLE || getAIState() == STATE_SCRIPTMOVE) && !m_moveTimer && !m_timeToMove && getUnitToFollow() == NULL) //creature is stopped and out of Combat
	{
		...
		if(m_formationLinkSqlId != 0)
		{
			if( m_formationLinkTarget == NULL)
			{
				...
				Creature* c = TO_CREATURE(GetUnit());
				if(!c->haslinkupevent)
				{
		...
		if(getUnitToFollow() == NULL)
		{
			...
			if ((GetUnit() && GetUnit()->GetMapMgr() && GetUnit()->GetMapMgr()->IsCollisionEnabled()) && sWorld.Collision)
			{
				...
				float ran = (rand()%(25*10))/10.0f;
				while(ran<1)
				{
				ran = (rand()%(25*10))/10.0f;
				}
...
uint8 AIInterface::CastSpell(Unit* caster, SpellEntry *spellInfo, SpellCastTargets targets)
{
	...
	Spell* nspell(new Spell(caster, spellInfo, false, NULLAURA));
...
{
	Unit* obj = GetUnit()->GetMapMgr()->GetUnit(guid);
	if(obj)
...
{
	if(GetUnit()==NULL || Slot > 6)
		return;

	m_totemspelltimer = 0xEFFFFFFF;
	Creature * ourslave= NULLCREATURE;
	
	ourslave = TO_CREATURE(GetUnit()->CreateTemporaryGuardian(TotemEntry,summon_duration,float(-M_PI*2), 0,Slot));
	if(ourslave != NULL)
	{
		//Just use slot 0, there shouldn't be any other spawns
		if(GetUnit()->m_SummonSlots[Slot] != NULL)
			GetUnit()->SummonExpireSlot(Slot);  // a previous summon?
		...
		ourslave->CalcDamage();
		ourslave->SetSummonOwnerSlot(GetUnit()->GetGUID(),Slot);

		GetUnit()->m_SummonSlots[Slot] = ourslave;
	}
}
...
void AIInterface::CallGuards()
{
	...
	if(  getMSTime() > m_guardTimer && !IS_INSTANCE(GetUnit()->GetMapId()))
	{
		...
		uint16 AreaId = GetUnit()->GetMapMgr()->GetAreaID(GetUnit()->GetPositionX(),GetUnit()->GetPositionY(),GetUnit()->GetPositionZ());
		...
		GetUnit()->SendChatMessage(CHAT_MSG_MONSTER_SAY, team ? LANG_ORCISH : LANG_COMMON, "Guards!");

		uint8 spawned = 0;
	
		unordered_set<Player*>::iterator hostileItr = GetUnit()->GetInRangePlayerSetBegin();
		for(; hostileItr != GetUnit()->GetInRangePlayerSetEnd(); ++hostileItr)
		{
			...
			guard = GetUnit()->GetMapMgr()->CreateCreature(guardId);
			if(guard == NULL)
				continue;
