...
void *ChainAggroEntity::operator new(size_t size)
{
	sLog.outDebug("New ChainAggroEntity dynamic string in object.");
	return malloc(size);
}

void ChainAggroEntity::operator delete(void* p)
{
	sLog.outDebug("Delete ChainAggroEntity dynamic string in object.");
	free(p);
}
