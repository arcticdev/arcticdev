...
void *VoiceChatHandler::operator new(size_t size)
{
	sLog.outDebug("New VoiceChatHandler dynamic string in object.");
	return malloc(size);
}

void VoiceChatHandler::operator delete(void* p)
{
	sLog.outDebug("Delete VoiceChatHandler dynamic string in object.");
	free(p);
}
