...
void *AuthSocket::operator new(size_t size)
{
	sLog.outDebug("New AuthSocket dynamic string in object.");
	return malloc(size);
}

void AuthSocket::operator delete(void* p)
{
	sLog.outDebug("Delete AuthSocket dynamic string in object.");
	free(p);
}
...
void AuthSocket::HandleChallenge()
{
	...
	if(full_size > sizeof(sAuthAuthChallenge_C))
	{
		Disconnect();
		return;
	}
	...
	if(build > AuthServer::getSingleton().max_build)
	{
		// wtf?
	...
	if(build < AuthServer::getSingleton().min_build)
	{
		// can we patch?
...
{
	if(GetReadBuffer().GetSize() < sizeof(sAuthAuthProof_C))
		return ;
	...
	if(m_patch&&!m_account)
	{
		...
		DEBUG_LOG("AuthAuthProof","Intitiating PatchJob");
	...
	DEBUG_LOG("AuthAuthProof","Interleaving and checking proof...");

	sAuthAuthProof_C lp;
	GetReadBuffer().Read(&lp, sizeof(sAuthAuthProof_C));
	...
	if(memcmp(lp.M1, M.AsByteArray(), 20) != 0)
	{
		...
		//SendProofError(4, 0);
		SendChallengeError(CE_NO_ACCOUNT);
		DEBUG_LOG("AuthAuthProof","M1 values don't match.");
		return;
	}
...
//	OUT_DEBUG("\n========================\n");
	...
	DEBUG_LOG("AuthAuthProof","Authentication Success.");
	...
	sAuthSQL->Execute("UPDATE accounts SET lastlogin=NOW(), SessionKey = '%s', lastip='%s' WHERE acct=%u;", m_sessionkey_hex, GetRemoteIP().c_str(), m_account->AccountId);
}
...
void AuthSocket::HandleReconnectChallenge()
{
	...
	// Copy the data into our cached challenge structure
	if((size_t)(full_size+4) > sizeof(sAuthAuthChallenge_C))
	{
		Disconnect();
		return;
	}
	...
	// Check client build.
	if(m_challenge.build > AuthServer::getSingleton().max_build ||
		m_challenge.build < AuthServer::getSingleton().min_build)
	{
		SendChallengeError(CE_WRONG_BUILD_NUMBER);
		return;
	}
...
void AuthSocket::HandleReconnectProof()
{
	...
	QueryResult * result = sAuthSQL->Query ("SELECT SessionKey FROM accounts WHERE acct = %u", m_account->AccountId);
	if(result)
	{
	...
	if(GetReadBuffer().GetSize() < sizeof(sAuthAuthProofKey_C))
		return;

	sAuthAuthProofKey_C lp;
	GetReadBuffer().Read(&lp, sizeof(sAuthAuthProofKey_C));
