...
{
	sLog.outDebug("New Account dynamic string in object.");
	return malloc(size);
}

void Account::operator delete(void* p)
{
	sLog.outDebug("Delete Account dynamic string in object.");
	free(p);
}

//Конструктор и деструктор для динамических строк в объектах
void *AccountMgr::operator new(size_t size)
{
	sLog.outDebug("New AccountMgr dynamic string in object.");
	return malloc(size);
}

void AccountMgr::operator delete(void* p)
{
	sLog.outDebug("Delete AccountMgr dynamic string in object.");
	free(p);
}

//Конструктор и деструктор для динамических строк в объектах
void *InformationCore::operator new(size_t size)
{
	sLog.outDebug("New InformationCore dynamic string in object.");
	return malloc(size);
}

void InformationCore::operator delete(void* p)
{
	sLog.outDebug("Delete InformationCore dynamic string in object.");
	free(p);
}
...
void AccountMgr::ReloadAccounts(bool silent)
{
	...
	// Load *all* accounts.
	QueryResult * result = sAuthSQL->Query("SELECT acct, login, password, gm, flags, banned, forceLanguage, muted FROM accounts");
	Field * field;
	string AccountName;
 ...
 */
	if(strcmp(field[6].GetString(), "ruRU"))
	{
	...
	if( m_encryptedPasswords )
	{
		...
		if( bn.GetNumBytes() != 20 )
		{
			...
			sAuthSQL->Execute("UPDATE accounts SET password = SHA1(CONCAT(UPPER(login), ':', UPPER(password))) WHERE acct = %u", acct->AccountId);
		}
		else
		{
			if ( Password.size() == 40 )
...
void AccountMgr::UpdateAccount(Account * acct, Field * field)
{
	...
	if ((uint32)UNIXTIME > acct->Banned && acct->Banned != 0 && acct->Banned > 3) //1 = perm ban?
	{
		...
		sAuthSQL->Execute("UPDATE accounts SET banned = 2 WHERE acct=%u",acct->AccountId); //expired ban -> freeze Account
	}
	acct->SetGMFlags(GMFlags.c_str());
	if(strcmp(field[6].GetString(), "ruRU"))
	{
...
BAN_STATUS IPBanner::CalculateBanStatus(in_addr ip_address)
{
	...
	for(; itr2 != banList.end();)
	{
		...
		if( ParseCIDRBan(ip_address.s_addr, itr->Mask, itr->Bytes) )
		{
			...
			{
				sAuthSQL->Execute("DELETE FROM ipbans WHERE expire = %u AND ip = \"%s\"", itr->Expire, sAuthSQL->EscapeString(itr->db_ip).c_str());
				banList.erase(itr);
			}
			else
			{
...
void IPBanner::Reload()
{
	...
	listBusy.Acquire();
	banList.clear();
	QueryResult * result = sAuthSQL->Query("SELECT ip, expire FROM ipbans");
	if( result != NULL )
	{
		do 
		{
...
void InformationCore::UpdateRealmPop(uint32 realm_id, float pop)
{
	...
	map<uint32, Realm*>::iterator itr = m_realms.find(realm_id);
	if(itr != m_realms.end())
		itr->second->Population = (pop > 0) ? ((pop < 3) ? pop : 3) : 0;
	realmLock.Release();
}
...
void InformationCore::SendRealms(AuthSocket * Socket)
{
	...
	*(uint16*)&data.contents()[1] = uint16(data.size() - 3);
	...
	// Refresh the realms pop. this is not good cuz we refresh all sockets(realms) pop,
	// or maybe it is good idk :s
	set<AuthCommServerSocket*>::iterator itr2;
	AuthCommServerSocket * s;
	for(itr2 = m_serverSockets.begin(); itr2 != m_serverSockets.end(); itr2++)
	{
		s = *itr2;
		s->RefreshRealmsPop();
	}
...
void InformationCore::TimeoutSockets()
{
	...
	set<AuthCommServerSocket*>::iterator itr, it2;
	AuthCommServerSocket * s;
...
void InformationCore::SetRealmOffline(uint32 realm_id, AuthCommServerSocket *ss)
{
	realmLock.Acquire();
	map<uint32,Realm*>::iterator itr = m_realms.find( realm_id );
	if( itr != m_realms.end() )
	{
		...
		{
			DEBUG_LOG("AuthServer","Realm: %s is now offline.\n", pr->Name.c_str());
...
void InformationCore::CheckServers()
{
	...
	for(itr = m_serverSockets.begin(); itr != m_serverSockets.end();)
	{
		...
		{
			DEBUG_LOG("AuthServer","Disconnecting socket: %s due to it no longer being on an allowed IP.\n", s->GetRemoteIP().c_str());
			s->Disconnect();
		}
	}
