...
void AuthServer::Run(int argc, char ** argv)
{
    ...
    Log.Notice("Config", "Checking Config File");

    if(!Config.MainConfig.SetSource(config_file, true))
	{
		Log.Error("Config File"," Can't find config file!");
		getchar();
	}
	...
	max_build = Config.MainConfig.GetIntDefault("Client", "MaxBuild", 9999);
