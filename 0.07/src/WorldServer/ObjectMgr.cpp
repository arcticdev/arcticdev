...
void *Charter::operator new(size_t size)
{
	sLog.outDebug("New Charter dynamic string in object.");
	return malloc(size);
}

void Charter::operator delete(void* p)
{
	sLog.outDebug("Delete Charter dynamic string in object.");
	free(p);
}

//Конструктор и деструктор для динамических строк в объектах
void *ObjectMgr::operator new(size_t size)
{
	sLog.outDebug("New ObjectMgr dynamic string in object.");
	return malloc(size);
}

void ObjectMgr::operator delete(void* p)
{
	sLog.outDebug("Delete ObjectMgr dynamic string in object.");
	free(p);
}
...
ObjectMgr::~ObjectMgr()
{
	Log.Notice("Destructor", "Deleting Corpses...");
	CorpseCollectorUnload();

	Log.Notice("Destructor", "Deleting Itemsets...");
	...
	Log.Notice("Destructor", "Deleting Level Information...");
	...
	Log.Notice("Destructor", "Deleting Waypoint Cache...");
	...
	Log.Notice("Destructor", "Deleting NPC Say Texts...");
	...
	Log.Notice("Destructor", "Deleting Charters...");
	...
	Log.Notice("Destructor", "Deleting Reputation Tables...");
	...
	Log.Notice("Destructor", "Deleting Groups...");
	...
	Log.Notice("Destructor", "Deleting GM Tickets...");
...
//	int total = sSkillStore.GetNumRows();
	...
	Log.Notice("DBC", "%u spell skills loaded.", mSpellSkills.size());
}
...
void ObjectMgr::LoadPlayersInfo()
{
	...
	if(result)
	{
		...
		do
		{
			...
			if( GetPlayerInfoByName(pn->name) != NULL )
			{
				...
				Log.Notice("Loader", "Renaming duplicate player %s to %s. (%u)", pn->name,temp,pn->guid);
	...
	Log.Notice("Database", "%u players loaded.", m_playersinfo.size());
	LoadGuilds();
}
...
void ObjectMgr::LoadPlayerCreateInfo()
{
	...
	Log.Notice("Database", "%u player create infos loaded.", mPlayerCreateInfo.size());
	GenerateLevelUpInfo();
}
...
void ObjectMgr::LoadGMTickets()
{
	...
	Log.Notice("Database", "%u GM Tickets loaded.", result->GetRowCount());
	delete result;
}
...
void ObjectMgr::SetHighestGuids()
{
	...
	Log.Notice("Database", "Total Corpses = %u", m_hiCorpseGuid);
	Log.Notice("Database", "Total Players = %u", m_hiPlayerGuid);
	Log.Notice("Database", "Total GameObjects = %u", m_hiGameObjectSpawnId);
	Log.Notice("Database", "Total Creatures = %u", m_hiCreatureSpawnId);
	Log.Notice("Database", "Total Items = %u", m_hiItemGuid);
	Log.Notice("Database", "Total Containers = %u", m_hiContainerGuid);
	Log.Notice("Database", "Total Groups = %u", m_hiGroupId);
	Log.Notice("Database", "Total Charters = %u", m_hiCharterId);
...
void ObjectMgr::LoadVendors()
{
	...
	if( result != NULL )
	{
		...
		{
			Log.Notice("Database", "Invalid format in vendors (%u/5) columns, not enough data to proceed.\n", result->GetFieldCount() );
			return;
		}
		else if( result->GetFieldCount() > 6 )
		{
			Log.Notice("Database", "Invalid format in vendors (%u/5) columns, loading anyway because we have enough data\n", result->GetFieldCount() );
		}
		...
		do
		{
			...
			if( ec != 0 )
			{
				...
				if( itm.extended_cost == NULL )
				{
					...
					Log.Warning("Database","Item %u at vendor %u has extended cost %u which is invalid. Skipping.", itm.itemid, fields[0].GetUInt32(), ec);
					continue;
				}
			}
	...
	Log.Notice("Database", "%u vendors loaded.", mVendors.size());
}
...
void ObjectMgr::LoadTotemSpells()
{
	...
	delete result;
	Log.Notice("Database", "%u totem spells loaded.", m_totemSpells.size());
}
...
void ObjectMgr::LoadTrainers()
{
	...
	do 
	{
		...
		if(!result2)
		{
			Log.Error("Trainers", "Trainer without spells, entry %u.", entry);
			if(tr->UIMessage)
				delete [] tr->UIMessage;
			delete tr;
			continue;
		...
		else
		{
			do
			{
				...
				if( ts.pCastSpell == NULL && ts.pLearnSpell == NULL )
				{
					...
					Log.Warning("Database", "Trainer %u skipped invalid spell (%u/%u).", entry, CastSpellID, LearnSpellID);
	...
	delete result;
	Log.Notice("Database", "%u trainers loaded.", mTrainers.size());
}
...
void ObjectMgr::GenerateLevelUpInfo()
{
	...
	Log.Notice("Loader", "%u level up informations generated.", mLevelInfo.size());
}
...
//	int i = 0;
	...
	delete result;
	Log.Notice("Database", "%u spell overrides loaded.", mOverrideIdMap.size());
}
...
void ObjectMgr::LoadCreatureWaypoints()
{
	...
	Log.Notice("Database", "%u waypoints cached.", result->GetRowCount());
	delete result;
}
...
void ObjectMgr::LoadGuildCharters()
{
	...
	delete result;
	Log.Notice("Database", "%u charters loaded.", m_charters[0].size());
}
...
void ObjectMgr::RemoveCharter(Charter * c)
{
	...
	{
		Log.Notice("Destructor", "Charter %u cannot be destroyed as type %u is not a sane type value.", c->CharterId, c->CharterType );
		return;
	}
	m_charterLock.AcquireWriteLock();
...
void ObjectMgr::LoadReputationModifierTable(const char * tablename, ReputationModMap * dmap)
{
	...
	Log.Notice("Database", "%u reputation modifiers on %s.", dmap->size(), tablename);
}
...
void ObjectMgr::LoadMonsterSay()
{
	...
	Log.Notice("Database", "%u monster say events loaded.", result->GetRowCount());
	delete result;
}
...
void ObjectMgr::LoadDisabledSpells()
{
	...
	Log.Notice("Database", "%u disabled spells.", m_disabled_spells.size());
}
...
void ObjectMgr::LoadGroups()
{
	...
	Log.Notice("Database", "%u groups loaded.", this->m_groups.size());
}
