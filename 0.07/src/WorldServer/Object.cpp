...
{
	sLog.outDebug("New Object dynamic string in object.");
	return malloc(size);
}

void Object::operator delete(void* p)
{
	sLog.outDebug("Delete Object dynamic string in object.");
	free(p);
}
...
//		flags = 0x18;
	...
	case TYPEID_PLAYER:
		...
		// gameobject/dynamicobject
	case TYPEID_GAMEOBJECT:
		flags = 0x0350; //VLack: got this from Aspire
		break;
	case TYPEID_DYNAMICOBJECT:
		flags = 0x0048; //VLack: got this from Aspire
		break;
	case TYPEID_CORPSE:
//		flags = 0x58;
		flags = 0x0148; //VLack: guess what, Aspire
		break;
...
//					flags = 0x5A;
		...
		//The above 3 checks FAIL to identify transports, thus their flags remain 0x58
		if( flags != 0x0352 && GetTypeId() == TYPEID_GAMEOBJECT && static_cast<GameObject*>(this)->GetInfo()->Type == GAMEOBJECT_TYPE_TRANSPORT )
			flags = 0x0352;
	}
...
void Object::DealDamage(UnitPointer pVictim, uint32 damage, uint32 targetEvent, uint32 unitEvent, uint32 spellId)
{
	...
	if(pVictim->GetTypeId() == TYPEID_UNIT && TO_CREATURE(pVictim)->GetCreatureName())
	{
		...
		else if(TO_CREATURE(pVictim)->proto)
		{
			...
			switch( TO_CREATURE(pVictim)->proto->Id)
			{
				case 1921:
				case 2673:
				case 2674:
...
bool Object::IsGiveXPorHonor(PlayerPointer plr, UnitPointer Target)
{
       ...
       if( Target->IsPlayer() )
              if( HonorHandler::CalculateHonorPointsForKill( plr , Target ) <= 0 )
                     return false;
       return true;
}
