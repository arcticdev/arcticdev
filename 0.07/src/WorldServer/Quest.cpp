...
void *QuestLogEntry::operator new(size_t size)
{
	sLog.outDebug("New QuestLogEntry dynamic string in object.");
	return malloc(size);
}

void QuestLogEntry::operator delete(void* p)
{
	sLog.outDebug("Delete QuestLogEntry dynamic string in object.");
	free(p);
}
