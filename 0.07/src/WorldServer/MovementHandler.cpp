...
{
	if(!_player->IsInWorld() || _player->m_uint32Values[UNIT_FIELD_CHARMEDBY] || _player->GetPlayerStatus() == TRANSFER_PENDING || _player->GetTaxiState() || _player->m_TransporterGUID)
		return;
	...
	{
		if(_player->m_TransporterGUID && !movement_info.transGuid.GetOldGuid())
		{
	...
	/************************************************************************/
	/* if( sWorld.antihack_flight && !( movement_info.flags & MOVEFLAG_SWIMMING || movement_info.flags & MOVEFLAG_FALLING || movement_info.flags & MOVEFLAG_FALLING_FAR || movement_info.flags & MOVEFLAG_FREE_FALLING ))
	{
		...
		{
			_player->BroadcastMessage( "Flyhack detected. In case the server is wrong then make a report how to reproduce this case. You will be logged out in 10 seconds." );
