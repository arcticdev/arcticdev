...
void *AddonMgr::operator new(size_t size)
{
	sLog.outDebug("New AddonMgr dynamic string in object.");
	return malloc(size);
}

void AddonMgr::operator delete(void* p)
{
	sLog.outDebug("Delete AddonMgr dynamic string in object.");
	free(p);
}
...
bool AddonMgr::IsAddonBanned(std::string name, uint64 crc)
{
	...
	if(itr != mKnownAddons.end())
	{
		...
		{
			sLog.outDebug(ADDONSISDANNEDAI, name.c_str());
			return true;
		}
	}
	else
	{
		...
		sLog.outDebug(DISCNEWADDFONSAI, name.c_str());
...
void AddonMgr::SendAddonInfoPacket(WorldPacket *source, uint32 pos, WorldSession *m_session)
{
	...
	{
		sLog.outDebug(WARLINGINCOPLEAI);
		return;
	}
	...
	if(result != Z_OK)
	{
		sLog.outError(DECOMPRESSIONSAI);
		return;
	}

	sLog.outDetail(DECOMPRESSIONSAI);
...
void AddonMgr::LoadFromDB()
{
	...
	if(!result)
	{
		sLog.outString(QUERYFAILEDSELAI); // schnek: sLog.outString("Query failed: SELECT * FROM clientaddons");
		return;
	}
...
{
	sLog.outString(ADDONMGRSAVINGAI);
	...
	for(itr = mKnownAddons.begin(); itr != mKnownAddons.end(); ++itr)
	{
		if(itr->second->isNew)
		{
			sLog.outDetail(SAVINGNEWADDONAI, itr->second->name.c_str());
			std::stringstream ss;
