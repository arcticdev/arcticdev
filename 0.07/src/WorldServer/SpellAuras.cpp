...
void *Aura::operator new(size_t size)
{
	sLog.outDebug("New Aura dynamic string in object.");
	return malloc(size);
}

void Aura::operator delete(void* p)
{
	sLog.outDebug("Delete Aura dynamic string in object.");
	free(p);
}
...
void Aura::SpellAuraSchoolAbsorb(bool apply)
{
	...
	if(apply)
	{
		...
		if( plr )
		{
			float otcoef = GetSpellProto()->Misc_coef;
			float ddcoef = GetSpellProto()->Base_coef;
			float coefmod = 0;
			...
			// For spells Affected by Bonus Healing we use Base_coef.
			if( ddcoef > 0 )
				val += float2int32( float( plr->HealDoneMod[GetSpellProto()->School] ) * ddcoef );
			// For spells Affected by Bonus Damage we use Misc_coef.
			else if( otcoef > 0 )
