			...
			if(sgr)
			{
				...
				for(itr = sgr->GetGroupMembersBegin(); itr != sgr->GetGroupMembersEnd(); ++itr)
				{
					...
					if(pPlayer && pPlayer->GetGUID() !=  pguid)
					{
						...
						uint32 response = 0;
						//Проверяем, можем ли мы взять квест
