...
#define PREMIUM_ACC_REST_EXP 3

UpdateMask Player::m_visibleUpdateMask;
#define COLLISION_MOUNT_CHECK_INTERVAL 1000
static const uint8 baseRunes[6] = {0,0,1,1,2,2};
static const uint32 DKNodesMask[12] = {4294967295,4093640703,830406655,0,33570816,1310944,3250593812,73752,896,67111952,0,0}; // schnek: all old continents are available to DK's by default. 

//Конструктор и деструктор для динамических строк в объектах
void *Player::operator new(size_t size)
{
	sLog.outDebug("New Player dynamic string in object.");
	return malloc(size);
}

void Player::operator delete(void* p)
{
	sLog.outDebug("Delete Player dynamic string in object.");
	free(p);
}
...
void Player::EventReduceDrunk(bool full)
{
	...
	if(drunk == 0) sEventMgr.RemoveEvents(plr_shared_from_this(), EVENT_PLAYER_REDUCEDRUNK);
}
...
void Player::SendDualTalentConfirm()
{
	...
	{
		// Не достаточно средств на счету :P
	...
	if(!init)	// unapply old spec
	{
		...
		if(GetSummon())
		{
			if(GetSummon()->GetUInt32Value(UNIT_CREATED_BY_SPELL) > 0)
				...
				GetSummon()->Remove(false, true, true);
		}
	}
...
void Player::CalcResistance(uint32 type)
{
	...
	// schnek: Динамическая аура 285, удаляет бонус.
	for( uint32 x = 0; x < MAX_AURAS; x++ )
	{
		if( m_auras[x] != NULL )
		{
	...
	// schnek: Динамическая аура 285, добавляет бонус.
	for( uint32 x = 0; x < MAX_AURAS; x++ )
	{
		if( m_auras[x] != NULL )
		{
...
{     /*
      sEventMgr.RemoveEvents(plr_shared_from_this(), EVENT_PLAYER_SOFT_DISCONNECT);
